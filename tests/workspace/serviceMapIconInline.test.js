...
test("workspace service map card renders its icon inline without a late-loading mask asset", () => {
  ...
  assert.match(componentSource, /const serviceMapMarkerMaskId = `\$\{serviceMapId\}-service-map-marker`/);
  assert.match(componentSource, /<mask id=\{serviceMapMarkerMaskId\}[\s\S]*?M19\.8 19\.68c-\.66 0-1\.34-\.34-1\.82-\.91/);
  assert.match(componentSource, /<g mask=\{`url\(#\$\{serviceMapMarkerMaskId\}\)`\}>[\s\S]*?M25\.07 29\.15c-\.51\.01-1\.09-\.72-1\.14-\.79/);
  ...
  assert.doesNotMatch(componentSource, /serviceMapLogoMaskId/);
