...
test("mono theme renders black and gray glass, icons, controls and home/about tokens", () => {
  ...
  assert.match(mono, /--mono-bg-top:\s*#2f2f2f/);
  assert.match(mono, /--mono-text:\s*#c8c8c8/);
  assert.match(mono, /--mono-title:\s*#c57171/);
  assert.match(mono, /--mono-icon:\s*rgba\(230,\s*230,\s*230/);
  assert.match(mono, /--glass-ring-surface-bg:\s*rgba\(20,\s*20,\s*20,\s*0\.62\)/);
  assert.match(mono, /--link-gold:\s*var\(--mono-title\)/);
  ...
  assert.match(mono, /\.home-about-title\s*\{[\s\S]*?color:\s*var\(--mono-title\) !important;/);
  assert.match(mono, /\.home-before-links \.home-quick-link,[\s\S]*?\.home-before-links \.home-quick-link svg\s*\{[\s\S]*?color:\s*var\(--mono-title\) !important;/);
  assert.match(mono, /\.homepage-root :is\([\s\S]*?\.home-link,[\s\S]*?\.home-scroll-cue-link,[\s\S]*?\.home-before-contact-copy a[\s\S]*?\)\s*\{[\s\S]*?color:\s*var\(--mono-title\) !important;[\s\S]*?-webkit-text-fill-color:\s*var\(--mono-title\) !important;/);
  assert.match(mono, /\.home-before-links \.home-quick-label\s*\{[\s\S]*?color:\s*var\(--mono-text\) !important;[\s\S]*?-webkit-text-fill-color:\s*var\(--mono-text\) !important;/);
  assert.match(mono, /\.guide-policy-scroll a,[\s\S]*?\.guide-rich-link[\s\S]*?\)\s*\{[\s\S]*?color:\s*var\(--mono-title\) !important;[\s\S]*?-webkit-text-fill-color:\s*var\(--mono-title\) !important;/);
  assert.match(mono, /--chat-tools-panel-bg:\s*var\(--mono-tooltip-surface\)/);
  assert.match(mono, /\.drawer-panel--chat-glass \.drawer-title\s*\{[\s\S]*?color:\s*var\(--mono-title\) !important;/);
  ...
  assert.match(mono, /--profile-logout-outer-stroke:\s*var\(--mono-title\)/);
  assert.match(mono, /--profile-logout-arrow-stroke:\s*var\(--mono-title\)/);
  assert.match(mono, /--profile-logout-label:\s*#c8c8c8/);
  assert.match(mono, /--mono-orbit-surface:/);
  assert.match(mono, /--mono-floating-surface:/);
  assert.match(mono, /--mono-tooltip-surface:/);
  assert.match(mono, /--chat-rail-tooltip-bg:\s*var\(--mono-tooltip-surface\)/);
  assert.doesNotMatch(mono, /--chat-rail-tooltip-bg:\s*var\(--mono-floating-surface\)/);
  assert.doesNotMatch(mono, /--chat-tools-panel-bg:\s*var\(--mono-floating-surface\)/);
  assert.match(mono, /--mono-input-surface:/);
  ...
  assert.match(mono, /--btn-primary-bg:\s*var\(--mono-orbit-surface\)/);
  assert.match(mono, /--btn-primary-text-hover:\s*var\(--mono-title-soft\)/);
  assert.match(mono, /--seg-card-text-hover:\s*var\(--mono-title-soft\)/);
  assert.match(mono, /--seg-card-text-selected:\s*var\(--mono-title-soft\)/);
  assert.match(mono, /--form-surface:\s*var\(--mono-input-surface\)/);
  assert.match(mono, /--input-bg:\s*var\(--form-surface\)/);
  assert.match(mono, /--chat-icon-dark:\s*var\(--mono-title\)/);
  assert.match(mono, /:root\.theme-mono:not\(\[data-contrast="hc"\]\) button:has\(\.back-icon-arrow\)[\s\S]*?--back-arrow-color:\s*var\(--mono-title\)/);
  assert.match(mono, /:root\.theme-mono:not\(\[data-contrast="hc"\]\) button:has\(\.back-icon-arrow\)[\s\S]*?--back-dot-color:\s*var\(--mono-title\)/);
  assert.match(mono, /:root\.theme-mono:not\(\[data-contrast="hc"\]\) \.chat-rail-icon-btn \.back-icon-dot \{[\s\S]*?fill:\s*var\(--mono-title\) !important;/);
  assert.match(mono, /--chat-tools-panel-bg:\s*var\(--mono-tooltip-surface\)/);
  assert.match(mono, /--chat-tools-item-hover-bg:\s*var\(--mono-tooltip-surface-hover\)/);
  assert.match(mono, /:root\.theme-mono:not\(\[data-contrast="hc"\]\) \.chat-tools-menu \{[\s\S]*?background:\s*var\(--mono-tooltip-surface\)/);
  assert.match(mono, /:root\.theme-mono:not\(\[data-contrast="hc"\]\) \.chat-tools-surface-popover\s*\{[\s\S]*?background:\s*var\(--mono-tooltip-surface\) !important;/);
  assert.match(mono, /:root\.theme-mono:not\(\[data-contrast="hc"\]\) #login-modal \.login-help-popover\s*\{[\s\S]*?background:\s*var\(--mono-tooltip-surface\) !important;[\s\S]*?color:\s*var\(--mono-text\) !important;/);
  assert.match(mono, /--mono-tooltip-surface:\s*[\s\S]*?linear-gradient\(180deg,\s*rgb\(48,\s*48,\s*48\)/);
  assert.match(mono, /--mono-tooltip-surface-hover:\s*[\s\S]*?rgb\(46,\s*46,\s*46\)/);
  ...
  assert.match(mono, /:root\.theme-mono:not\(\[data-contrast="hc"\]\) \.materials-page-shell\s*\{[\s\S]*?--subpage-card-bg:\s*var\(--mono-input-surface\) !important;[\s\S]*?--input-bg:\s*var\(--mono-input-surface\) !important;/);
  assert.match(mono, /:root\.theme-mono:not\(\[data-contrast="hc"\]\) \.workspace-dashboard-panel \.materials-page-body\s*\{[\s\S]*?--subpage-card-bg:\s*var\(--mono-input-surface\) !important;[\s\S]*?--input-bg:\s*var\(--mono-input-surface\) !important;/);
  assert.match(mono, /:root\.theme-mono:not\(\[data-contrast="hc"\]\) \.workspace-dashboard-panel \.materials-page-body :is\([\s\S]*?\.materials-comment-glow-field\.ui-glow-field,[\s\S]*?\.materials-comment-box,[\s\S]*?\.materials-comment-glow-field \.ui-glow-control[\s\S]*?\)\s*\{[\s\S]*?background:\s*var\(--mono-input-surface\) !important;[\s\S]*?background-image:\s*var\(--mono-input-surface\) !important;/);
  ...
  assert.match(mono, /:root\.theme-mono:not\(\[data-contrast="hc"\]\) \.workspace-feature-dropdown:not\(\.pre-inquiry-dropdown\) \.documents-dropdown-item\s*\{[\s\S]*?background:\s*rgb\(28,\s*28,\s*28\) !important;[\s\S]*?color:\s*var\(--mono-text\) !important;/);
  ...
  assert.match(mono, /:root\.theme-mono:not\(\[data-contrast="hc"\]\) \.workspace-feature-panel[\s\S]*?--workspace-feature-accent:\s*var\(--mono-title\)/);
  assert.match(mono, /:root\.theme-mono:not\(\[data-contrast="hc"\]\) \.drawer-panel--chat-glass :is\(\.drawer-close-btn--chat, \.drawer-close-btn--chat > span\)\s*\{[\s\S]*?color:\s*var\(--mono-title\) !important;/);
  assert.match(infoButton, /:global\(:root\.theme-mono:not\(\[data-contrast="hc"\]\)\) \.trigger \{[\s\S]*?--page-info-ring-color:\s*var\(--mono-title,\s*#c57171\);[\s\S]*?--page-info-dot-color:\s*var\(--mono-title,\s*#c57171\);/);
  assert.match(infoButton, /:global\(:root\.theme-mono:not\(\[data-contrast="hc"\]\)\) \.closeButton \{[\s\S]*?color:\s*var\(--mono-title,\s*#c57171\);/);
  // Rail tooltip theming is shared by both rails via composes; it now lives
  // once in rail.module.css.
  assert.match(rail, /:global\(:root\.theme-mono:not\(\[data-contrast="hc"\]\)\) \.tooltip \{[\s\S]*?color:\s*var\(--mono-text,\s*#c8c8c8\);[\s\S]*?background:\s*linear-gradient\(180deg,\s*rgb\(48,\s*48,\s*48\)/);
  assert.match(workspacePanel, /:global\(:root\.theme-mono\) \.cardIcon\s*\{[\s\S]*?color:\s*var\(--mono-title,\s*#c57171\);/);
  ...
  assert.match(workspacePanel, /:global\(:root\.theme-mono\) \.cardTitle\s*\{[\s\S]*?color:\s*var\(--mono-text,\s*#c8c8c8\);/);
  assert.match(orbital, /:root\.theme-mono[\s\S]*?var\(--mono-text,\s*#c8c8c8\)/);
  ...
  assert.match(orbital, /:root\.theme-mono:not\(\[data-contrast="hc"\]\)[\s\S]*?\.profile-orbit-item-label[\s\S]*?color:\s*var\(--mono-text,\s*#c8c8c8\) !important;/);
  assert.match(orbital, /:root\.theme-mono:not\(\[data-contrast="hc"\]\)[\s\S]*?\.profile-orbit-item-icon[\s\S]*?color:\s*var\(--mono-title,\s*#c57171\) !important;/);
  assert.match(orbital, /var\(--mono-orbit-surface/);
  assert.match(loginModal, /const isMonoTheme = prefs\?\.theme === "mono";/);
  assert.match(loginModal, /isMonoTheme[\s\S]*?var\(--mono-orbit-surface/);
  ...
  assert.match(loginModal, /"--otp-copy-text": isMidTheme[\s\S]*?isMonoTheme[\s\S]*?var\(--mono-text,\s*#c8c8c8\)/);
...
test("app code no longer carries legacy forest theme tokens", () => {
  const files = [
    "app/styles/theme/mono.css",
    ...
    "app/styles/features/chat/shell.css",
    ...
    "components/chat/rail.module.css",
  ...
  const reads = files.map((path) =>
    readFile(new URL(`../../${path}`, import.meta.url), "utf8").then((content) => [path, content])
  );

  return Promise.all(reads).then((entries) => {
    const legacyUses = entries
      .filter(([, content]) => /--forest-/.test(content))
      .map(([path]) => path);

    assert.deepEqual(legacyUses, []);
  });
});
