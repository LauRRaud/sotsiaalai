...
test("profile update pages wrap visible inputs in dark-mode BorderGlow fields", () => {
  ...
  assert.match(glowField, /glowRadius=\{42\}/);
  assert.match(glowField, /glowIntensity=\{0\.62\}/);
  ...
  assert.match(glassCss, /var\(--btn-primary-shadow-hover\)[\s\S]*?rgba\(255,\s*122,\s*126,\s*0\.38\)/);
  ...
  assert.match(glassCss, /html\[data-contrast="hc"\]\s+:is\(\.ui-glow-field,\s*\.ui-glow-button-frame,\s*\.ui-glow-option-card-frame\)[\s\S]*?--glow-color:\s*rgba\(255,\s*234,\s*0,\s*0\.64\)\s*!important/);
  assert.match(glassCss, /html\[data-contrast="hc"\]\s+\.ui-glow-button-frame:hover:not\(\.ui-glow-button-frame--disabled\)[\s\S]*?rgba\(255,\s*234,\s*0,\s*0\.72\)/);
  assert.doesNotMatch(glassCss, /:root:not\(\.theme-light\):not\(\.theme-mid\)\s+\.ui-glow-field:focus-within/);
  assert.match(glassCss, /rgba\(255,\s*122,\s*126,\s*0\.38\)/);
