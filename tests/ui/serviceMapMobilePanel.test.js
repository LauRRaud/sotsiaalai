...
test("service map mobile filter panel stays within the viewport when opened", () => {
  ...
  assert.match(styles, /@media \(max-width: 768px\) \{[\s\S]*?\.service-map-workspace \{[\s\S]*?--service-map-map-radius:\s*0;/);
...
test("service map page does not render decorative background layer", () => {
  const backgroundLayer = read("components/backgrounds/BackgroundLayer.jsx");

  assert.match(backgroundLayer, /const BACKGROUND_LAYER_EXCLUDED_PATHS = new Set\(\[\s*["']\/teenusekaart["']/);
  assert.match(backgroundLayer, /if \(BACKGROUND_LAYER_EXCLUDED_PATHS\.has\(normalizedPathname\)\) return null;/);
});
