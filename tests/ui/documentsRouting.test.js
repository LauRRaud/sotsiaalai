...
test("workspace hides the documents library card in client view", () => {
  const workspacePanel = read("components/chat/WorkspacePanel.jsx");
  ...
  assert.match(
    nonProviderBranch,
    /\.\.\.\(!isClientView\s*\?\s*\[\[\s*makeCard\(\{\s*key:\s*"documents"[\s\S]*?key:\s*"document_drafting"[\s\S]*?\]\]\s*:\s*\[\[\s*makeCard\(\{\s*key:\s*"document_drafting"/
  );
});
