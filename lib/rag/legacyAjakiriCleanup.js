...
export function buildLegacyAjakiriCleanupPlan(documents = []) {
  const articleDocs = [];
  const legacyDocs = [];
    ...
    .map(legacy => {
      ...
      if (!replacement) {
        return {
          ...
          reason: candidates.length > 1 ? "ambiguous_replacement" : "replacement_not_found",
      ...
      return {
        ...
        source_type: legacy.source_type,
        source_file_type: legacy.source_file_type,
        reason: "legacy_file_replaced_by_article_ingest",
