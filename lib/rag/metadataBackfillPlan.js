...
function buildSourceId(source = {}, root = {}, context = {}) {
  const existing = firstValue(source.source_id, source.sourceId);
  if (existing) return slugifyRagId(existing);
  const base = firstValue(source.key, source.id, source.title);
  if (!base) return null;
...
function sourceStatus(source = {}) {
  return firstValue(
    ...
    source.sourceStatus,
    source.itemType ? mapKovItemStatusToRagSourceStatus(source.status) : null,
    "active"
  );
}
...
function buildBackfilledMetadata(sourceInput = {}, context = {}, options = {}) {
  ...
  const checkedAt = dateOnly(firstValue(source.last_checked, source.lastChecked, root.checkedAt, root.last_checked, options.defaultLastChecked));

  return {
    ...source,
    source_id: sourceId,
    document_id: firstValue(source.document_id, source.documentId, sourceId),
    title,
    ...
    valid_from: dateOnly(firstValue(source.valid_from, source.validFrom)),
    valid_to: dateOnly(firstValue(source.valid_to, source.validTo)),
    historical: source.historical === true,
    source_status: sourceStatus(source),
    url_canonical: firstValue(source.url_canonical, source.urlCanonical, source.url, source.sourceUrl),
