...
export function normalizeRagSourceMetadata(record = {}, options = {}) {
  ...
  const canonical = {
    ...
    source_type: toTrimmedString(firstValue(
      merged.source_type,
      merged.sourceType,
      root.source_type,
      root.sourceType,
    ...
    authority: toTrimmedString(firstValue(merged.authority, root.authority)),
    ...
    collection_id: toTrimmedString(firstValue(merged.collection_id, merged.collectionId, root.collection_id, root.collectionId)),
    ...
    source_urls: Array.isArray(firstValue(merged.source_urls, merged.sourceUrls, root.source_urls, root.sourceUrls))
      ? [...new Set(firstValue(merged.source_urls, merged.sourceUrls, root.source_urls, root.sourceUrls).map(item => String(item || "").trim()).filter(Boolean))]
      : [],
