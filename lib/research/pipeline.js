...
function renderReportText(report, locale, targetStyle) {
  ...
  const refsTitle = locale === "en" ? "Sources" : locale === "et" ? "Allikad" : i18n.refs;
  const refPrefix = i18n.refPrefix || "source";
  const pagePrefix = i18n.pagePrefix || "lk";
  ...
  if (!report.findings.length) {
    ...
    for (const finding of report.findings) {
      const refs = finding.evidence_refs
        .map(ref => evidenceIndexById.get(String(ref || "").trim()) || String(ref || "").trim())
        .filter(Boolean)
        .join(", ");
      const refPart = refs ? `; ${refPrefix}: ${refs}` : "";
      lines.push(`- ${finding.claim} (${i18n.confidence}: ${formatConfidence(finding.confidence, locale)}${refPart})`);
    }
  }
  ...
  if (Array.isArray(report.evidence) && report.evidence.length) {
    lines.push("", `${refsTitle}`);
    for (const [index, ev] of report.evidence.entries()) {
      const title = ev.title || ev.docId || ev.id;
      const pagePart = ev.pageRange ? `, ${pagePrefix} ${ev.pageRange}` : "";
      lines.push(`${index + 1}. ${title}${pagePart}`);
    }
  }
  return lines.join("\n").trim();
}
