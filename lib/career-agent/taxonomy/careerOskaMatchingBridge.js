  ...
  return {
    occupation: occupation
      ? {
          ...
          externalUrl: occupation.externalUrl || null,
          occupationGroup: occupation.occupationGroup || null,
          localizedLabels: occupation.localizedLabels || null,
          localCatalog: occupation.localCatalog || null,
        }
      : null,
...
export async function suggestRelatedDirectionsWithOska(
  directions = [],
  taxonomyService,
  options = {}
) {
  ...
  const maxResultsPerDirection = Number.isFinite(options.maxResultsPerDirection)
    ? options.maxResultsPerDirection
    : 4;
  const minScore = Number.isFinite(options.minOccupationScore)
    ? options.minOccupationScore
    : 0.45;
  ...
  for (const direction of toSafeArray(directions)) {
    const title = getDirectionTitleValue(direction);
    if (!title) continue;

    for (const match of await service.searchOccupations(title, {
      limit: maxResultsPerDirection,
      minScore,
    })) {
      const key = `${coerceString(match.code) || ""}:${normalizeLookupText(match.label)}`;
      if (!match?.label || seen.has(key)) continue;
      seen.add(key);

      suggestions.push({
        direction: {
          title: match.label,
          type: direction?.type || "job",
          priority: Math.max(8, (Number(direction?.priority) || 12) - 2),
          rationale: uniqueStrings(toSafeArray(direction?.rationale)),
          missingRequirements: uniqueStrings(
            toSafeArray(direction?.missingRequirements)
          ),
        },
        oska: buildOskaMeta(match, null, []),
      });
    }
  }
