...
export function createCareerTaxonomyService(config = {}) {
  ...
  async function refreshTaxonomy(options = {}) {
    ...
    try {
      ...
      const baseDataset = normalizeOskaDataset(rawTaxonomy);
      const localCatalog = await loadCareerLocalOccupationCatalog();
      const dataset = mergeLocalOccupationsIntoDataset(baseDataset, localCatalog, {
        locale,
      });
