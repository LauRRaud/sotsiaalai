...
function unsetPathMutable(target, path) {
...
function preserveUserMetaFieldsForCvPatch(profile, patch) {
  const safePatch =
    patch && typeof patch === "object" ? deepClone(patch) : null;

  if (!safePatch) return patch;
  ...
  for (const path of protectedPaths) {
    const existingField = getValueAtPath(profile, path);
    if (getMetaSource(existingField) === PROFILE_SOURCES.FROM_USER) {
      unsetPathMutable(safePatch, path);
    }
  }
...
function buildProfile(payload = {}) {
  ...
  if (canonicalPatch) {
    const patchSource = inferPatchSource(payload);
    const patchToMerge =
      patchSource === PROFILE_SOURCES.FROM_CV
        ? preserveUserMetaFieldsForCvPatch(profile, canonicalPatch)
        : canonicalPatch;

    profile = mergeProfilePatch(profile, patchToMerge, {
      defaultSource: patchSource,
    });
  }
