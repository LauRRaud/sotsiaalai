...
function getLatestDraftText(runtime = {}) {
  ...
  if (
    normalized === "ei ole lisada midagi" ||
    normalized === "ei ole midagi lisada" ||
    normalized === "pole midagi lisada" ||
    normalized === "ei ole midagi rhutada" ||
    normalized === "pole midagi rhutada" ||
    normalized === "nothing to add" ||
    normalized === "no additional info" ||
    normalized === "nothing else to add"
  ) {
    return null;
  }
...
function containsAnyKeyword(source = "", keywords = []) {
  return keywords.some((keyword) => source.includes(keyword));
}

function buildHeuristicDesiredDirectionCandidates(profile = {}, runtime = {}, options = {}) {
  ...
  const sourceTexts = uniqueStrings([
    ...
    coerceString(runtime.latestUserText),
    coerceString(runtime.userMessage),
    ...getListItems(profile?.selfAnalysis?.interests, []),
    ...getListItems(profile?.selfAnalysis?.developmentNeeds, []),
    ...getListItems(profile?.skills?.digitalSkills, []),
    ...getListItems(profile?.experience?.sectors, []),
  ]);

  const normalizedPool = normalizeSearchText(
    sourceTexts
      .map((item) => getDirectionTitleValue(item) || coerceString(item))
      .filter(Boolean)
      .join(" ")
  );
  const hasAiSignal = containsAnyKeyword(normalizedPool, [
    "tehisintellekt",
    "masinpe",
    "masinope",
    "andme",
  ]);
  const hasItSignal = containsAnyKeyword(normalizedPool, [
    " it ",
    "ikt",
    "infotehnoloogia",
    "tarkvara",
  ...
  const hasDevelopmentSignal = containsAnyKeyword(normalizedPool, [
    "arend",
    "rakend",
    "susteem",
    "platvormi",
  ]);
  const hasAnalysisSignal = containsAnyKeyword(normalizedPool, [
    "analuut",
    "andme",
    "uuring",
    "modelleer",
  ]);
  const hasDesignSignal = containsAnyKeyword(normalizedPool, [
    "teenusedisain",
    "disain",
    "protsessiarendus",
  ]);
  const hasSocialDomainSignal = containsAnyKeyword(normalizedPool, [
    "sotsiaal",
    "hoolekanne",
    "kogukonna",
    "noust",
  ...
  if (hasAiSignal || hasItSignal || hasDevelopmentSignal) {
    titles.push("IKT-süsteemide analüütik");
    titles.push("Tarkvaraarendaja");
  }

  if (hasAiSignal || hasAnalysisSignal) {
    titles.push("Andmeanalüütik");
  }

  if (hasSocialDomainSignal && (hasAiSignal || hasItSignal || hasDevelopmentSignal)) {
    titles.push("Teenusedisainer");
  }
  ...
  return uniqueStrings(titles).map((title, index) => ({
    title,
    type: "job",
    priority: Math.max(19, 24 - index),
    rationale: [text.direction.desiredTargetRationale || "See suund põhineb sinu enda kirjeldatud eesmärgil."],
    missingRequirements: [],
  }));
}

function buildDesiredDirectionCandidates(profile = {}, runtime = {}, options = {}) {
  const text = getOrchestratorLocaleText(options);
  const sourceTexts = uniqueStrings([
    coerceString(runtime.problemStatementText),
    coerceString(runtime.profileBackgroundText),
  ...
  const seen = new Set();
  return [
    ...buildHeuristicDesiredDirectionCandidates(profile, runtime, options),
    ...explicitCandidates,
  ].filter((item) => {
...
function applyDirectionOpportunitySignals(rankedOpportunities = []) {
  ...
  return safeRanked
    .map((item) => {
      ...
      const blendedScore =
        (Number(item?.score) || 0) * 0.55 + normalizedPriority * 0.45;
...
async function resolveDirectionItems(profile, options = {}) {
  ...
  const desiredDirections = buildDesiredDirectionCandidates(
    profile,
    options.runtime,
    options
  );
