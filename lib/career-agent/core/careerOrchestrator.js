...
import { getCareerOrchestratorText } from "../i18n/careerOrchestratorText.js";
...
function buildRuntimeIntentFlags(
  ...
  return {
    userMessageProvided: runtime.userMessageProvided === true || Boolean(userText),
    requiresJobMatching:
...
function buildSummaryPayload(profileSummary, modeDecision, rankedOpportunities, actionPlan, directionItems) {
  return {
    ...
    mainDirection: getTopDirectionTitle(
      { directions: {} },
      rankedOpportunities,
      directionItems
    ),
...
async function resolveDirectionItems(profile, options = {}) {
  ...
  const sourceDirections = explicitDirections.length > 0 ? explicitDirections : profileDirections;
...
function shouldReturnAgreementBlock(state, modeDecision) {
  return (
    state === CAREER_AGENT_STATES.AGREEMENTS &&
    modeDecision.deniedConsentKeys.length > 0
  );
}
...
function pickQuestionLimit(modeDecision, options = {}) {
  if (Number.isFinite(options.questionLimit)) return options.questionLimit;
  return modeDecision?.interactionRules?.maxQuestionsPerTurn || 2;
}
...
function buildQuestionOrConfirmationResponse(state, profile, runtime, modeDecision, options = {}) {
  ...
  const questions = getQuestionsForState(state, profile, runtime, {
    ...
    limit: pickQuestionLimit(modeDecision, options),
    locale,
  });
  ...
  if (state === CAREER_AGENT_STATES.CONFIRM_PROFILE) {
    const profileSummary = summarizeProfileForConfirmation(profile);
...
export async function resolveCareerTurn({
  ...
  const currentState =
    safeRuntime.currentState || getRecommendedEntryState(safeProfile, safeRuntime);
  ...
  if (shouldReturnAgreementBlock(currentState, modeDecision)) {
    return {
      profile: safeProfile,
      runtime: safeRuntime,
      modeDecision,
  ...
  if (toSafeArray(opportunities).length > 0) {
    if (shouldRequireJobMatchingConsent({ ...options, opportunities })) {
    ...
    rankedOpportunities = opportunityResolution.ranked;
