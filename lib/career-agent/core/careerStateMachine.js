...
function getRequiredAgreementConsentKeys(profile, runtime = {}) {
  ...
  if (runtime?.requiresProfileStorage === true) {
    keys.push("profileStorageApproved");
  }
...
export function isStateSatisfied(state, profile, runtime = {}) {
  ...
  switch (state) {
    ...
    case CAREER_AGENT_STATES.AGREEMENTS:
      if (context.agreements.requiredConsentKeys.length === 0) {
        return true;
      }
      return (
    ...
    case CAREER_AGENT_STATES.PARSE_PROFILE:
      return (
        ...
        context.runtime.profileParsed === true
      );
    ...
    case CAREER_AGENT_STATES.SHORTLIST_DIRECTIONS:
      return (
        context.runtime.shortlistPresented ||
