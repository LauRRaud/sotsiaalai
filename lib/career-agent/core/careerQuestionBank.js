...
function normalizeStringList(value) {
  if (Array.isArray(value)) {
    ...
    if (
      items.length === 1 &&
      /^(ei|ei\.|eip|none|no|puudub|pole|n\/a)$/i.test(items[0])
    ) {
      return [];
    }
    return items;
...
function resolveLocalizedPrompt(question, localized = {}, fallback = {}, profile, runtime) {
  if (question?.id === "profile_background_summary") {
    const hasCv =
      profile?.sourceMode?.cvUploaded === true ||
      runtime?.profileCvAvailable === true;

    const variantKey = hasCv ? "promptWithCv" : "promptWithoutCv";
    const variantPrompt =
      (typeof localized?.[variantKey] === "string" &&
        localized[variantKey].trim() &&
        localized[variantKey]) ||
      (typeof fallback?.[variantKey] === "string" &&
        fallback[variantKey].trim() &&
        fallback[variantKey]) ||
      null;
...
function localizeQuestion(question, locale = "et", profile = {}, runtime = {}) {
  ...
  const localizedPrompt = resolveLocalizedPrompt(
    question,
    localized,
    fallback,
    profile,
...
export function getQuestionsForState(state, profile, runtime = {}, options = {}) {
  ...
  return questions
    .slice(0, limit)
    .map((item) => localizeQuestion(item, locale, profile, runtime));
}
