...
const CONTACT_QUESTIONS = [
  question({
    id: "contact_preferred_channel",
    state: CAREER_AGENT_STATES.CONTACT,
    group: "contact",
    type: QUESTION_TYPES.SINGLE_SELECT,
    targetPath: "contact.preferredChannel",
    prompt: "Mis kanalis on sulle kõige mugavam nõustamist jätkata?",
    required: true,
    priority: 10,
    options: [
      { value: "chat", label: "Vestlus siin samas" },
  ...
  question({
    ...
    priority: 20,
    when: (profile) => getMetaValue(profile?.contact?.preferredChannel) === "email",
  }),
  question({
    id: "contact_phone",
    state: CAREER_AGENT_STATES.CONTACT,
    ...
    priority: 30,
    when: (profile) => getMetaValue(profile?.contact?.preferredChannel) === "phone",
  }),
];
