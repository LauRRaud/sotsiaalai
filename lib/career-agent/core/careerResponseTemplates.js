...
export function buildDirectionShortlistResponse(directions = [], options = {}) {
  ...
  const cards = safeDirections.map((item) => ({
    ...
    code: item?.code || item?.oska?.occupation?.code || null,
    field: item?.field || item?.oska?.occupation?.localCatalog?.field || null,
    group:
      item?.group ||
      item?.oska?.occupation?.occupationGroup ||
      item?.oska?.occupation?.parentLabel ||
      null,
    linkUrl: item?.linkUrl || item?.oska?.occupation?.externalUrl || null,
    studyPlaces: takeTop(
      item?.studyPlaces || item?.oska?.occupation?.studyPlaces || [],
      5
    ),
...
export function buildOptionAnalysisResponse(analysis = [], options = {}) {
  ...
  const cards = safeAnalysis.map((item) => ({
    ...
    code: item?.opportunity?.oska?.occupation?.code || null,
    field:
      item?.opportunity?.oska?.occupation?.localCatalog?.field ||
      item?.opportunity?.oska?.field?.label ||
      item?.opportunity?.sector ||
      null,
    group:
      item?.opportunity?.oska?.occupation?.occupationGroup ||
      item?.opportunity?.oska?.occupation?.parentLabel ||
      null,
    linkUrl: item?.opportunity?.oska?.occupation?.externalUrl || null,
    studyPlaces: takeTop(item?.opportunity?.oska?.occupation?.studyPlaces || [], 5),
...
export function buildSummaryResponse(summary = {}, options = {}) {
  const text = getLocaleText(options);
  const hasDistinctMainDirection =
    summary?.mainDirection &&
    summary?.mainDirection !== summary?.recommendation;

  return makeResponse({
    kind: CAREER_RESPONSE_KINDS.SUMMARY,
    state: options.state || CAREER_AGENT_STATES.SUMMARY,
    title: options.title || text.summary.title,
    message: options.message || text.summary.message,
    bullets: uniqueStrings([
      summary?.recommendation
        ? `${text.summary.labels.recommendation}: ${summary.recommendation}`
        : null,
      summary?.recommendationReason
        ? `${text.summary.labels.recommendationReason}: ${summary.recommendationReason}`
        : null,
      summary?.goal ? `${text.summary.labels.goal}: ${summary.goal}` : null,
      hasDistinctMainDirection
        ? `${text.summary.labels.mainDirection}: ${summary.mainDirection}`
        : null,
      summary?.familiarPath
        ? `${text.summary.labels.familiarPath}: ${summary.familiarPath}`
        : null,
      summary?.firstStep ? `${text.summary.labels.firstStep}: ${summary.firstStep}` : null,
      toSafeArray(summary?.studyPlaces).length > 0
        ? `${text.summary.labels.studyPlaces}: ${toSafeArray(summary.studyPlaces)
            .slice(0, 3)
            .join(", ")}`
        : null,
