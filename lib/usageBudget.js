...
export const COST_STT_PER_AUDIO_MB_EUR = readNumber(process.env.ANALYTICS_COST_STT_PER_AUDIO_MB_EUR, 0.0075);
export const COST_TTS_PER_1K_CHARS_EUR = readNumber(process.env.ANALYTICS_COST_TTS_PER_1K_CHARS_EUR, 0.013);
...
export function estimateUsageCostEur(usage = {}) {
  ...
  const sttAudioBytes = Number(usage.sttAudioBytes || 0);
  const ttsChars = Number(usage.ttsChars || 0);

  const sttAudioMb = sttAudioBytes > 0 ? sttAudioBytes / (1024 * 1024) : 0;
  const chatEur = chatRequests * COST_CHAT_REQUEST_EUR;
  const ragEur = ragSearches * COST_RAG_SEARCH_EUR;
  const sttEur = sttAudioMb * COST_STT_PER_AUDIO_MB_EUR;
  const ttsEur = (ttsChars / 1000) * COST_TTS_PER_1K_CHARS_EUR;
...
export async function getUserMonthlyBudgetSnapshot(userId, options = {}) {
  const budgetEur = Number(options.budgetEur ?? MONTHLY_COST_BUDGET_EUR_PER_USER);
