...
function buildOutputSummary(fields, result) {
  const interruptionMap = [
    "Katkestuste kaart",
    "",
    `Katkestuse liik: ${fields.interruptionClass || "täpsustamisel"}.`,
    "Allikad:",
    ...list(fields.sources, "täpsustada katkestuse allikas"),
    `Sagedus: ${fields.frequency || "täpsustamisel"}.`,
    `Mõju tööle: ${fields.workImpact || "täpsustamisel"}.`,
    `Kohe reageerimise selgus: ${fields.immediateResponseNeed || "täpsustamisel"}.`,
    `Oodata võiks: ${fields.canWait || "täpsustamisel"}.`
  ].join("\n");

  const focusTimeAgreement = [
    "Fookusaja kokkulepe",
    "",
    `Osapool: ${fields.counterpart || "täpsustamisel"}.`,
    "Ettepanek: leppida kokku kindel fookusaeg, mille ajal katkestused lähevad ainult selgelt kiireloomulise kriteeriumi alusel.",
    `Vajalik kokkulepe: ${fields.neededAgreement || "täpsustamisel"}.`
  ].join("\n");
  ...
  const channelAgreement = [
    ...
    `Valest kanalist tulev osa: ${fields.wrongChannelShare || "täpsustamisel"}.`,
  ...
  const managerMemo = [
    ...
    `Peamine katkestuse liik: ${fields.interruptionClass || "täpsustamisel"}.`,
    `Kokkuleppe osapool: ${fields.counterpart || "täpsustamisel"}.`,
    `Vajalik kokkulepe: ${fields.neededAgreement || "täpsustamisel"}.`
  ].join("\n");
