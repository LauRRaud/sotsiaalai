...
function buildOutputSummary(fields, result) {
  ...
  const neutralSummary = [
    ...
    "",
    `Juhtumi tüüp: ${fields.caseType || "täpsustamisel"}.`,
    ...
    `Tööalane roll: ${fields.professionalRole || "täpsustamisel"}.`,
    `Peamine koormus: ${fields.mainLoad || "täpsustamisel"}.`,
  ...
  const managerMemo = [
    ...
    `Peamine teema: ${fields.mainLoad || "täpsustamisel"}.`,
    `Mis vajab tuge: ${fields.shouldNotCarryAlone ? "juhtumit ei peaks üksi kandma" : "järgmine samm vajab täpsustamist"}.`,
    `Soovitud 24h järeltegevus: ${Array.isArray(fields.next24hNeeds) ? fields.next24hNeeds.join(", ") : "täpsustamisel"}.`
  ].join("\n");
  ...
  const covisionInput = [
    ...
    `Mis teeb olukorra keeruliseks: ${fields.mainLoad || "täpsustamisel"}.`,
