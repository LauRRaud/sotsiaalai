...
export const GENERAL_RETENTION_DAYS = Math.max(1, Number(process.env.DATA_RETENTION_DAYS || 90));
export const PAYMENT_RETENTION_DAYS = Math.max(
  GENERAL_RETENTION_DAYS,
  Number(process.env.PAYMENT_RETENTION_DAYS || 7 * YEAR_DAYS)
);
export const PAYMENT_RAW_RETENTION_DAYS = Math.max(
  GENERAL_RETENTION_DAYS,
  Number(process.env.PAYMENT_RAW_RETENTION_DAYS || GENERAL_RETENTION_DAYS)
);
const RETENTION_SWEEP_INTERVAL_MS = Math.max(
  5 * 60 * 1000,
  Number(process.env.RETENTION_SWEEP_INTERVAL_MS || 6 * 60 * 60 * 1000)
);
