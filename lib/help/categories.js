...
export function inferHelpCategoryCode(input = {}) {
  ...
  if (category === "TRANSPORT" || /\b(transport|sõid|soit|viia|autoga|saatm[a-z]*)\b/i.test(text)) return "TRANSPORT";
  if (category === "IGAPAEVAABI" || /\b(pood|poes|ostud|igapäeva|igapaeva|errand|shopping)\b/i.test(text)) return "DAILY_TASKS";
  if (category === "KODUABI" || /\b(koduabi|korist|majapid|household|cleaning)\b/i.test(text)) return "HOME_HELP";
  if (category === "DIGIABI" || /\b(digi|telefon|arvuti|internet|e-teenus|computer|digital)\b/i.test(text)) return "DIGITAL_HELP";
  if (category === "TUGI JA HOOLDUS" || /\b(hoold|abistaja|care|support worker|isiklik)\b/i.test(text)) return "CARE_SUPPORT";
