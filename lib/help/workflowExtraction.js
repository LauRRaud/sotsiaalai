export function createHelpWorkflowExtraction({
  ...
  function applyQuestionFallbacks({ draft, message, activeQuestionLayer, activeQuestionKey }) {
    ...
    if (activeQuestionKey === "targetGroupCodes" && substantive && !draft.targetGroupCodes.length && !draft.targetGroups.length) {
    ...
    if (activeQuestionKey === "helpCompensation" && substantive && !draft.compensationDetails && draft.helpType && draft.helpType !== "VOLUNTARY") {
    ...
    if (activeQuestionLayer === "enrichment" && substantive) {
      ...
      } else if (String(activeQuestionKey || "").startsWith("create_help_request:")) {
        draft.conditions = trimmed;
      }
    }
  }
