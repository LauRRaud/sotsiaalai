export function createHelpWorkflowExtraction({
  ...
  function splitIntoSnippets(text = "") {
    const raw = stripTemplatePlaceholders(text);
    if (!raw) return [];
    const parts = raw
      .split(/\n+/)
  ...
  function cleanNaturalDescription(text = "") {
    return stripTemplatePlaceholders(text)
  ...
  function extractLocationCandidates(message = "") {
    const text = stripTemplatePlaceholders(message);
    if (!text) return [];
  ...
  async function mergeDraftState(state, message, { prismaClient, getNextMissingField } = {}) {
    ...
    const compensationDetails = detectScopedCompensationDetails(message, helpType || draft.helpType);
    ...
    const availabilityOrStart = detectScopedAvailabilityOrStart(message);
    ...
    const providerScopeOrConditions = detectScopedProviderScopeOrConditions(message);
    ...
    const categoryInfo = inferRichCategoryFromText(`${message} ${draft.description}`, draft.categoryCode);
