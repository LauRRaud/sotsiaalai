export function createHelpWorkflowExtraction({
  ...
  function detectScopedAvailabilityOrStart(message = "") {
    ...
    return pickSnippet(trimmed, (value) => {
      ...
      const hasTimingCue = WEEKDAY_OR_TIME_PATTERN.test(value)
        || /(kohe|lahiajal|paindlik|saadaval|saadavus|vaba|kokkuleppel|kell\s*\d|esmasp|teisip|kolmap|neljap|reede|laup|puhap|toopaev[a-z]*|paevadel)/.test(normalized);
      if (!hasTimingCue) return false;
      if (
        /(tasu eest|tasuline|tasu|olukorrast|olukorrale)/.test(normalized)
        && !/(kell\s*\d|esmasp|teisip|kolmap|neljap|reede|laup|puhap|hommik|ohtu|õhtu|paindlik|saadaval|saadavus|kohe|lahiajal|toopaev[a-z]*|paevadel)/.test(normalized)
      ) {
        return false;
      }
  ...
  function inferTargetGroupsFromMessage(message = "", previousCodes = []) {
    ...
    if (
      ...
      return {
        targetGroupCodes: ["CHILD", "YOUTH", "ADULT", "ELDER", "DISABILITY"],
