...
function _isEditSignal(text = "") {
  return /\b(muuda|muudan|edit|change|paranda|tegelikult|pigem)\b/.test(normalizeText(text));
}

function extractExplicitDraftEdit(message = "", intent = "create_help_request") {
  const trimmed = collapseWhitespace(message);
  if (!trimmed) return null;

  const leadIn = "(?:palun\\s+)?(?:võiks\\s+muuta|voiks\\s+muuta|soovin\\s+muuta|muuda|muudan|paranda|edit|change)?";
  const separator = "(?:\\s*(?:[:;,.-]|on|oleks|uueks)\\s*)?";
  const patterns = [
    {
      field: intent === "create_help_offer" ? "providerScopeOrConditions" : "conditions",
      names: "(?:eri)?tingimused|lisatingimused|praktilised\\s+tingimused"
    },
    {
      field: "availabilityOrStart",
      names: "saadavus|aeg|ajad|millal"
    },
    {
      field: "compensationDetails",
      names: "tasu\\s*info|tasu|sõidukulu|soidukulu|kuluhüvitis|kuluhuvitis"
    },
    {
      field: "title",
      names: "pealkiri|title"
    },
    {
      field: "description",
      names: "kirjeldus|sisu|description"
    }
  ];

  for (const pattern of patterns) {
    const match = trimmed.match(new RegExp(`^\\s*${leadIn}\\s*(?:${pattern.names})${separator}(.+)$`, "iu"));
...
function applyExplicitDraftEdit(state, message) {
  const edit = extractExplicitDraftEdit(message, state?.intent || "create_help_request");
  if (!edit) return null;
  ...
  draft.structuredSummary = generatePracticalStructuredSummary(draft, state?.municipalityLabel || "");
...
function normalizeDraft(input = {}) {
  ...
  const explicitTargetGroupCodes = uniqueStrings(input.targetGroupCodes).map((value) => value.toUpperCase());
  const targetGroupCodes = explicitTargetGroupCodes.length
    ? explicitTargetGroupCodes
    : inferTargetGroupCodes({ targetGroup, targetGroups });
  ...
  return {
    ...
    availabilityOrStart: stripAvailabilityHelpTypeTail(input.availabilityOrStart),
...
function generateTitleFromDraft(draft = {}, intent = "create_help_request") {
  ...
  if (explicitOrExistingTitle && !canReplaceGeneratedTitle) {
...
function buildQuestionReply(state, question, changedFields = [], replyLang = "et") {
  return [
    buildChangeReflection(state, changedFields, replyLang) || (changedFields.length ? buildSummarySentence(state, replyLang) : ""),
    question?.prompt || ""
  ].filter(Boolean).join("\n\n");
}
...
async function handleCreateTurn({ state, message, userId, replyLang, prismaClient, aiDraftPatcher }) {
  ...
  if (state.step === "preview" || state.step === "edit_or_save") {
    const explicitEdit = applyExplicitDraftEdit(state, message);
    if (explicitEdit) {
      return {
        handled: true,
        workflowState: explicitEdit.state,
        reply: buildPreviewReply(explicitEdit.state, replyLang, {
          prefix: buildChangeReflection(explicitEdit.state, [explicitEdit.field], replyLang)
        }),
        cards: [],
        attachments: []
      };
