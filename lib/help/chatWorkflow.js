...
import { buildBroadPrompt, buildEntryConfirmationReply, createHelpQuestionFlow } from "./workflowQuestions.js";
...
function _detectContactPreference(message = "") {
  const normalized = normalizeText(message);
...
function _looksLikeFieldOnlyAnswer(text = "") {
  const normalized = normalizeText(text);
...
async function handleCreateTurn({ state, message, userId, replyLang, prismaClient }) {
  ...
  if (!state.flowLocked || state.step === "intent_confirmation") {
    ...
    return {
      handled: true,
      workflowState: prepared.nextState,
      reply: buildQuestionReply(prepared.nextState, prepared.nextQuestion, changedAfterSource, replyLang),
      cards: [],
      attachments: []
    };
  }
