...
import { buildRoomChatPath } from "../roomPath.js";
  ...
  return results.map((item, index) => {
    ...
    return {
      ...
      hint: item.kind === "offer"
        ? helpWorkflowT(replyLang, "browse.cardHintOffer", { index: index + 1 })
        : helpWorkflowT(replyLang, "browse.cardHintRequest", { index: index + 1 })
    };
  });
}
  ...
  try {
    ...
    return {
      ...
      attachments: match.roomId
        ? [{
            label: helpWorkflowT(replyLang, "connect.openRoom"),
            url: buildRoomChatPath(match.roomId)
          }]
        : []
    };
  } catch (error) {
...
export async function runHelpChatWorkflow(input = {}, prismaClient = prisma) {
  ...
  if (!currentState) {
    ...
    if (!forcedIntent && (detectedIntent === "create_help_offer" || detectedIntent === "create_help_request")) {
