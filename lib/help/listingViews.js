...
function resolveTargetGroupLabels(record, locale = "et") {
  if (Array.isArray(record?.targetGroupLabels)) {
    return record.targetGroupLabels.map((item) => normalizeTargetGroupLabel(item, locale)).filter(Boolean);
  }

  const fromLinks = Array.isArray(record?.targetGroupLinks)
    ? record.targetGroupLinks.map((item) => normalizeTargetGroupLabel(pickLocalizedField(item?.targetGroup, locale), locale))
    : [];
  const fromTargetGroups = Array.isArray(record?.targetGroups)
    ? record.targetGroups.map((item) => normalizeTargetGroupLabel(pickLocalizedField(item, locale), locale))
    : [];
