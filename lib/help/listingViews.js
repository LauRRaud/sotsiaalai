...
function resolveSummary(record, title = "") {
  const explicitSummary = normalizeText(record?.summary || record?.structuredSummary || "");
  if (explicitSummary && explicitSummary !== normalizeText(title)) return truncateText(explicitSummary, 240);

  return truncateText(record?.description || "", 240);
}
