...
const ALLOWED_TARGET_GROUP_CODES = new Set(["CHILD", "YOUTH", "ADULT", "ELDER", "DISABILITY"]);
...
const TARGET_GROUP_LABELS = Object.freeze({
  ...
  ADULT: "Taiskasvanu",
  ELDER: "Eakas",
  DISABILITY: "Puue voi erivajadus"
});
...
export function isHelpAiExtractorEnabled(env = process.env) {
  return enabled(env.HELP_WORKFLOW_AI_EXTRACTOR);
}

export function getHelpAiExtractorModel(env = process.env) {
...
function allowedPatchFields(activeField = "", state = {}) {
  ...
  if (key === "categoryCode") fields.add("categoryCode");
  ...
  if (layer === "enrichment") {
    ...
    if (state?.intent === "create_help_offer") fields.add("providerScopeOrConditions");
