...
function stableEntryId(item, fallbackSlug) {
  ...
  const safe = normalizeSlug(raw || `${fallbackSlug}-${item?.title || item?.name || "contact"}`);
...
function mapContactToEntry(item, { municipality, packageData, folderSlug }) {
  ...
  return {
    ...
    type,
    title: clean(item?.title || item?.name || item?.department) || "KOV kontakt",
...
export async function syncKovContactsToServiceMap({
  ...
  const files = await listKovDataFiles(kovRoot);
  const result = {
  ...
  for (const file of files) {
    const packageData = await readJson(file.dataPath);
    const municipalitySlug = normalizeSlug(packageData?.municipality_id || file.folderSlug);
    const municipality = await prisma.municipality.findFirst({
      where: {
        OR: [
          { slug: file.folderSlug },
          { slug: municipalitySlug },
          { displayName: { equals: clean(packageData?.municipality_name || packageData?.municipality) || "", mode: "insensitive" } }
        ]
      },
      select: {
        id: true,
