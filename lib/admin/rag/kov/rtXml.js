...
function buildChunkText({ act, chapter, paragraph, subsection = null, point = null, subsectionIntro = "" }) {
  const lines = [];
  lines.push(`${act.municipality} - ${act.actTitle}`);
...
export function parseRtRegulationXml(xmlText, options = {}) {
  ...
  const municipality = normalizeInlineWhitespace(
    options.municipality || municipalityFromIssuer(issuer, options.displayName || "")
  );
        ...
        chunks.push({
          ...
          metadata: {
            ...buildParagraphMetadata(act, chapter, paragraph, {
              ...
              title: `${act.municipality} - ${act.actTitle} - ${SECTION_SIGN} ${paragraph.number || ""}`.trim(),
          ...
          chunks.push({
            ...
            metadata: {
              ...buildParagraphMetadata(act, chapter, paragraph, {
                ...
                title: `${act.municipality} - ${act.actTitle} - ${SECTION_SIGN} ${paragraph.number || ""} lg ${subsection.number || ""}`.trim(),
                section: chapter?.title || chapter?.number || null
              })
            }
          });
          ...
          chunks.push({
            ...
            metadata: {
              ...buildParagraphMetadata(act, chapter, paragraph, {
                ...
                chunk_level: "point",
                title: `${act.municipality} - ${act.actTitle} - ${SECTION_SIGN} ${paragraph.number || ""} lg ${subsection.number || ""} p ${point.number || ""}`.trim(),
                section: chapter?.title || chapter?.number || null
              })
            }
          });
