...
export function resolveKovRtManifestPath(root = "KOV") {
  return path.resolve(process.cwd(), root, "kov_rt", "kov_rt_manifest.json");
}
...
export async function resolveKovRtXmlFile(root = "KOV", entry = {}) {
  ...
  const slug = clean(entry.slug).toLowerCase();
  const folder = path.join(rootPath, slug);
  const rtFolder = path.join(folder, "rt");
  const xmlFile = clean(entry.xml_file);
  const warnings = [];
  const errors = [];

  if (!(await pathExists(folder))) {
    return { status: "skipped", xml_found: false, xml_path: null, warnings, errors: [`KOV folder missing: ${slug}`] };
  }

  if (xmlFile) {
    const flatPath = path.join(folder, xmlFile);
    if (await pathExists(flatPath)) return { status: "ready", xml_found: true, xml_path: flatPath, xml_file: xmlFile, warnings, errors };
    const nestedPath = path.join(rtFolder, xmlFile);
    if (await pathExists(nestedPath)) return { status: "ready", xml_found: true, xml_path: nestedPath, xml_file: xmlFile, warnings, errors };
  }

  const flatXml = await listXmlFiles(folder);
  const nestedXml = await listXmlFiles(rtFolder);
  const all = [
    ...flatXml.map(name => ({ name, filePath: path.join(folder, name) })),
    ...nestedXml.map(name => ({ name, filePath: path.join(rtFolder, name) }))
  ];
...
export async function auditKovRtManifestEntry(root = "KOV", entry = {}) {
  ...
  if (xmlResolution.xml_found && xmlResolution.xml_path) {
    try {
      const xmlText = await fs.readFile(xmlResolution.xml_path, "utf8");
  ...
  return {
    ...
    ingest_status: ready ? "ingest_ready" : errors.length ? "needs_review" : "skipped",
    warnings,
    errors,
    rag_payload_preview: {
...
export async function validateKovRtSlug(root = "KOV", slug = "") {
  ...
  const rootPath = path.resolve(process.cwd(), root);
  const normalizedSlug = clean(slug).toLowerCase();
  const kovFolder = path.join(rootPath, normalizedSlug);
  const coreFiles = ["sources.json", "json", "meta.json", "rag.md"].map(suffix => `${normalizedSlug}.${suffix}`);
  const coreFileResults = {};
  for (const fileName of coreFiles) coreFileResults[fileName] = await pathExists(path.join(kovFolder, fileName));
