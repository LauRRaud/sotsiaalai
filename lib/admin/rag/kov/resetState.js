...
export async function planKovRagStateResetBySlug(slug) {
  ...
  const registryMatches = matchedDocumentsForMunicipality(registry.records, municipality, "registry");
  const apiMatches = matchedDocumentsForMunicipality(documentsApi.records, municipality, "documents_api");
  ...
  const activeSnapshots = snapshots.filter(row => row.active === true);
