...
function packageForService(service = {}, snapshots = []) {
  const id = clean(service.id);
  return arrayValue(snapshots).find(snapshot =>
    clean(snapshot.canonicalItemId || snapshot.canonical_item_id) === id ||
...
export function buildJogevaFormsContactsAudit(input = {}) {
  ...
  const snapshotCanonicalIds = new Set(snapshotRows.map(snapshot => clean(snapshot.canonicalItemId || snapshot.canonical_item_id)).filter(Boolean));
  const services = localItems
    .filter(isServiceItem)
    .filter(item => !snapshotCanonicalIds.size || snapshotCanonicalIds.has(clean(item.id)));
