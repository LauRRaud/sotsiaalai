...
export function normalizeJogevaCanonicalItemId(value) {
...
function packageForService(service = {}, snapshots = []) {
  const id = normalizeJogevaCanonicalItemId(service.id);
  return arrayValue(snapshots).find(snapshot =>
    normalizeJogevaCanonicalItemId(snapshot.canonicalItemId || snapshot.canonical_item_id) === id ||
...
export function buildJogevaFormsContactsAudit(input = {}) {
  ...
  const snapshotCanonicalIds = new Set(snapshotRows
    .map(snapshot => normalizeJogevaCanonicalItemId(snapshot.canonicalItemId || snapshot.canonical_item_id))
    .filter(Boolean));
  const services = localItems
    .filter(isServiceItem)
    .filter(item => !snapshotCanonicalIds.size || snapshotCanonicalIds.has(normalizeJogevaCanonicalItemId(item.id)));
