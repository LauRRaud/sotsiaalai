...
export async function assistPreInquiry({
  ...
  const detectedUrgencyLevel = normalizedUrgencyLevel || detectUrgencyLevel(
    normalizedTopic,
    normalizedSituation,
    normalizedAssistantMessage
  );
  const suggestedNextSteps = inferSuggestedNextSteps({
    suggestions,
  ...
  const message = buildPreInquiryAssistantMessage({
    ...
    situationSummary,
    normalizedMunicipality,
    detectedUrgencyLevel: normalizedUrgencyLevel,
    suggestedNextSteps
  });
  ...
  return {
    ...
    suggestedNextSteps,
    reasoningText,
    recommendedRecipients: suggestions,
