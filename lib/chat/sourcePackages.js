...
const KOV_CANONICAL_RELATION_ROOTS = {
  jogeva_vald: ["KOV", "Jogeva", "jogeva-vald"]
};
...
function canonicalItemId(entry = {}) {
  return normalizeSourcePackageCanonicalItemId(entry.canonical_item_id || entry.canonicalItemId);
}
...
function loadKovCanonicalRelations(muniId) {
  ...
  const root = path.join(process.cwd(), ...KOV_CANONICAL_RELATION_ROOTS[id]);
  const slugName = KOV_CANONICAL_RELATION_ROOTS[id][KOV_CANONICAL_RELATION_ROOTS[id].length - 1];
  const data = readJsonIfExists(path.join(root, `${slugName}.json`));
  const sourcesData = readJsonIfExists(path.join(root, `${slugName}.sources.json`));
