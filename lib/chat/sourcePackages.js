...
function allowedSectionsForEntry(entry = {}) {
  if (hasSourceType(entry, JOURNAL_SOURCE_TYPES)) return [];
  if (hasSourceType(entry, LEGAL_BASIS_SOURCE_TYPES)) return ["legal_basis"];
  if (hasSourceType(entry, FORM_SOURCE_TYPES)) return ["forms"];
  if (hasSourceType(entry, CONTACT_SOURCE_TYPES)) return ["contacts"];
  ...
  if (hasSourceType(entry, SERVICE_SOURCE_TYPES) || type === "service" || type === "benefit" || type === "toetus") {
    ...
    if (hints.length) return hints.filter(section => !["legal_basis", "forms", "contacts"].includes(section));
...
function safeSectionSource(entry = {}, index = 0) {
  return Object.fromEntries(Object.entries({
    ...
    source_status: sourceStatus(entry),
...
export function buildRuntimeSourcePackages(entries = []) {
  ...
  entries.forEach((entry, index) => {
    ...
    const sections = allowedSectionsForEntry(entry);
