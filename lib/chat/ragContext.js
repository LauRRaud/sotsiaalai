...
export function groupMatches(matches) {
  ...
  matches.forEach((raw, idx) => {
    ...
    const groupKey = m.articleId || m.docId || (m.title ? `${m.title}|${m.fileName || ""}` : m.id || `match-${idx}`);
...
export function renderOneContextBlock(entry, index) {
  ...
  const issueYear = jTitle && (issueText || entry.year) ? [jTitle, issueText || entry.year].filter(Boolean).join(" ") : issueText || entry.year || jTitle || null;
  const headerParts = [];
  ...
  const bodyText = fullBody.length > RAG_GROUP_BODY_MAX_CHARS ? `${fullBody.slice(0, RAG_GROUP_BODY_MAX_CHARS - 1)}…` : fullBody;
