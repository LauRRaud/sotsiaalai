...
export function normalizeMatch(m, idx) {
  ...
  const tags =
    (Array.isArray(md.tags) && md.tags) ||
    (typeof md.tags === "string" ? md.tags.split(/[,;]/).map((s) => s.trim()).filter(Boolean) : null) ||
...
export function diversifyGroupsMMR(groups, k = CONTEXT_GROUPS_MAX, lambda) {
  ...
  const tokenize = (s) =>
    new Set(String(s || "").toLowerCase().split(/[^a-z0-9]+/i).filter(Boolean));
...
export function makeShortRef(entry, pagesCompact) {
  ...
  const fallbackName =
    ...
    (typeof entry.url === "string" ? entry.url.replace(/^https?:\/\/(www\.)?/, "").trim() : "");
...
export function renderOneContextBlock(entry, index) {
  ...
  const bodyText =
    fullBody.length > RAG_GROUP_BODY_MAX_CHARS ? `${fullBody.slice(0, RAG_GROUP_BODY_MAX_CHARS - 1)}…` : fullBody;
