...
export function groupMatches(matches) {
  ...
  matches.forEach((raw, idx) => {
    ...
    const shouldKeepChunkSeparate =
      ((m.collectionId === "kov_services" && m.sourceType === "municipality_kov") ||
        ((m.collectionId === "kov_regulations" || m.collectionId === "national_regulations") &&
          m.sourceType === "riigiteataja_regulation")) &&
      (m.itemId || m.chunkId);
...
function scoreSourceQuality(group) {
...
export function rankGroupsWithTopicHints(groups, topicHints = []) {
  if (!Array.isArray(groups) || groups.length === 0) return [];
  return groups
    .map(group => {
      const topicBoost = scoreTopicHintMatch(group, topicHints);
      const qualityAdjust = scoreSourceQuality(group);
...
export function selectTemporalGroups(groups, years = [], k = CONTEXT_GROUPS_MAX, lambda) {
  ...
  for (const year of targetYears) {
    ...
    const yearCandidates = remaining.filter(group => inferGroupYear(group) === year);
...
export function renderOneContextBlock(entry, index, options = {}) {
  ...
  if (yearText) headerParts.push(yearText);
...
function buildCompactContextWithBudget(groups, budget) {
  const usableGroups = (Array.isArray(groups) ? groups : []).slice(0, CONTEXT_GROUPS_MAX);
...
export function buildContextWithBudget(groups, options = {}) {
  ...
  const usableGroups = groups.slice(0, CONTEXT_GROUPS_MAX);
  const preferredYears = Array.from(new Set((Array.isArray(options?.preferredYears) ? options.preferredYears : [])
    .map(year => Number(year))
    .filter(year => Number.isInteger(year) && year >= 1900 && year <= 2100)));

  if (preferredYears.length >= 2 && usableGroups.length > 1) {
    return buildCompactContextWithBudget(usableGroups, budget);
  }
