...
export function buildSourceAttribution(reply = "", sources = [], options = {}) {
  ...
  if (list.length === 1) {
    ...
    if (!evidence.ok) {
      const decisions = [buildDecision(list[0], 0, "hide", "insufficient_evidence_strength", 0, evidence)];
      return buildAttributionResult(list, decisions, []);
    }
    const decisions = [buildDecision(list[0], 0, "display", "single_candidate_kept", 0, evidence)];
  ...
  const decisions = list.map((source, index) => {
    if (!sourceMatchesQueryAnchor(query, source)) {
      return buildDecision(source, index, "hide", "query_anchor_mismatch", 0);
    }
    const score = scoreSourceForReply(reply, source);
    if (score < threshold) {
      return buildDecision(source, index, "hide", "weak_reply_overlap", score);
    }
    const evidence = sourceMeetsEvidenceRequirement(source, riskPolicy);
    if (!evidence.ok) {
      return buildDecision(source, index, "hide", "insufficient_evidence_strength", score, evidence);
    }
    displayItems.push({ source, index, score });
    return buildDecision(source, index, "display", "reply_overlap_validated", score, evidence);
  });
