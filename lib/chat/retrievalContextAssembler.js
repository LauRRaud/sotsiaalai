...
function selectMunicipalityServiceBenefitGroups(groups = [], k = CONTEXT_GROUPS_MAX) {
  const selected = [];
  const seen = new Set();
  const add = (items) => {
...
export async function assembleRetrievalContext({
  ...
  if (shouldRunRag) {
    ...
    chosen = temporalRetrievalPlan.enabled
      ...
      : municipalityServiceBenefitListRequest
        ? selectMunicipalityServiceBenefitGroups(groupedMatches, Math.max(CONTEXT_GROUPS_MAX, 14))
    ...
    budgeted = buildContextWithBudget(
      ...
      temporalRetrievalPlan.enabled
        ...
        : municipalityServiceBenefitListRequest
          ? {
              compact: true,
              maxGroups: Math.max(CONTEXT_GROUPS_MAX, 14)
            }
          : undefined
    );
  }
