...
function buildLegalCitationInstruction(replyLang = "et") {
  if (replyLang === "en") {
    return [
      ...
      "Do not invent paragraph numbers or quote legal wording that is not visible in RAG_CONTEXT.",
      "If the retrieved legal context is only general or incomplete, say which part is confirmed and which exact provision is not visible."
    ].join("\n");
  }
  if (replyLang === "ru") {
    return [
  ...
  return [
    "LEGAL_CITATION_MODE:",
    "Kui vastad seaduse, määruse või korra kohta, nimeta täpne paragrahvinumber ja paragrahvi pealkiri, kui need on RAG_CONTEXT-is näha.",
    "Ära mõtle paragrahvinumbreid välja ega tsiteeri õiguslikku sõnastust, mida RAG_CONTEXT ei sisalda.",
    "Kui leitud õiguslik kontekst on ainult üldine või puudulik, ütle, mida see kinnitab ja millist täpset sätet näha ei ole."
  ].join("\n");
}
