...
function isMunicipalityServiceBenefitListRequest(message = "") {
  const normalized = normalizeIntentText(message);
  if (!normalized) return false;
  const asksList = /\b(loetle|nimeta|too valja|too välja|millised|mis|nimekiri|ulevaade|ülevaade)\b/.test(normalized);
...
export async function assembleRetrievalContext({
  ...
  const municipalityServiceBenefitListRequest =
    allowMunicipalityScopedRag && isMunicipalityServiceBenefitListRequest(effectiveMessage);
