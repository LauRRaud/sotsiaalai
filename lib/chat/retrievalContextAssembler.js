...
function packageDisplayedSourcesFromPackages(sourcePackages = [], allowedIds = [], existingSources = []) {
  const allowed = new Set((Array.isArray(allowedIds) ? allowedIds : [])
    .map(value => String(value || "").trim())
    .filter(Boolean));
  if (!allowed.size) return [];

  const seen = new Set((Array.isArray(existingSources) ? existingSources : [])
    .map(stableSourceIdFromDisplaySource)
    .filter(Boolean));
  const out = [];
    ...
    for (const [section, list] of Object.entries(sections)) {
      ...
      for (const source of list) {
        ...
        out.push({
          ...
          url: (source.url || source.source_url || source.url_canonical || source.official_url || source.officialUrl)
            ? displayUrl(source.url || source.source_url || source.url_canonical || source.official_url || source.officialUrl)
            : undefined,
...
export async function assembleRetrievalContext({
  ...
  if (shouldRunRag) {
    ...
    chosen = legalLookupPlan?.enabled && legalLookupPlan.mode === "explicit_paragraph"
      ? legalSelection.selectionGroups.slice(0, contextGroupTarget)
      ...
      : municipalityServiceBenefitListRequest
        ...
        : broadMultiSourceQuestion
          ? selectMultiSourceGroups(groupedMatches, contextGroupTarget, DIVERSIFY_LAMBDA)
        : diversifyGroupsMMR(groupedMatches, contextGroupTarget, DIVERSIFY_LAMBDA);
  ...
  if (packageAwareAnsweringUsed) {
    extraSystemInstructions.push(
      "Kasuta KOV teenuse või toetuse vastamisel SourcePackage'i esmase struktuurina. Ära väida puuduvat vormi, kontakti, õiguslikku alust, tasu ega tähtaega, kui package märgib selle sektsiooni puuduvaks."
    );
  }
  ...
  if (packageAwareAnsweringUsed) {
    sources = [
      ...sources,
      ...packageDisplayedSourcesFromPackages(
        sourcePackages,
