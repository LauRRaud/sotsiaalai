...
function shouldCarryMunicipalityFromHistory(message = "") {
  ...
  const socialHelpFollowup =
    /\b(loetle|nimeta|too valja|too välja|millised|mis|teenus|teenused|teenuseid|toetus|toetused|toetusi|abi|sotsiaalabi|sotsiaalteenus|sotsiaalteenused)\b/.test(normalized);
...
function isMunicipalityServiceBenefitListRequest(message = "") {
  ...
  const asksServicesOrBenefits = /\b(teenus|teenused|teenuseid|toetus|toetused|toetusi|sotsiaalabi|abi liigid)\b/.test(normalized);
  ...
  return queryEntries.flatMap((entry) =>
    municipalityNames.flatMap((municipalityName) => {
      ...
      return [
        ...
        {
          query: [municipalityName, "toetused sotsiaaltoetus toimetulekutoetus sissetulekust soltuv toetus toetuse taotlus"].join("\n"),
          filters: mergeQueryFilters(entry, {
            collection_id: "kov_services",
            item_type: "benefit"
          })
