...
export async function assembleRetrievalContext({
  ...
  if (shouldRunRag && (isGraphChannelEnabled() || graphChannelTestOverride === true)) {
    try {
      const graphLookup = await graphChannelLookup({ question: effectiveMessage, prisma });
      const graphQueries = graphHintsToQueryTexts(graphLookup);
      if (graphQueries.length > 0) primaryRagQueries.push(...graphQueries);
      graphChannel = {
        matched_entities: graphLookup.matched_entities,
        hint_group_count: (graphLookup.hints || []).length,
        added_query_count: graphQueries.length
      };
