...
function isMunicipalityServiceBenefitListRequest(message = "") {
  ...
  const asksServicesOrBenefits = /\b(teenus|teenused|teenuseid|toetus|toetused|toetusi|sotsiaalabi|abi liigid|sotsiaalteenus|sotsiaalteenused|sotsiaalteenuseid|sotsiaaltoetus|sotsiaaltoetused|sotsiaaltoetusi)\b/.test(normalized);
  return asksList && asksServicesOrBenefits;
}

function detectServiceBenefitIntent(message = "") {
  const normalized = normalizeIntentText(message);
  const wantsServices = /\b(teenus|teenused|teenuseid|sotsiaalteenus|sotsiaalteenused|sotsiaalteenuseid)\b/.test(normalized);
  const wantsBenefits = /\b(toetus|toetused|toetusi|sotsiaaltoetus|sotsiaaltoetused|sotsiaaltoetusi)\b/.test(normalized);
...
function buildMunicipalityScopedQueries(ragQueries = [], municipalities = [], options = {}) {
  ...
  return queryEntries.flatMap((entry) =>
    municipalityNames.flatMap((municipalityName) => {
      ...
      if (wantsServices) {
        expanded.push({
          query: [municipalityName, "sotsiaalteenused teenused koduteenus tugiisik sotsiaaltransport eluruum volanoustamine lapsehoid hooldus"].join("\n"),
      ...
      if (wantsBenefits) {
        expanded.push({
          query: [municipalityName, "toetused sotsiaaltoetus sotsiaaltoetused sotsiaaltoetusi toimetulekutoetus vajaduspohine toetus hooldajatoetus eestkostetoetus matusetoetus tervisetoetus sissetulekust soltuv toetus toetuse taotlus"].join("\n"),
          filters: mergeQueryFilters(entry, {
            collection_id: "kov_services",
            item_type: "benefit"
          })
...
function buildMunicipalityListInstruction(replyLang = "et") {
  ...
  return [
    ...
    "Eralda teenused toetustest. Kui toetuste voi teenuste pool ei ole leitud kontekstis piisavalt nahtav, utle seda, ara leiuta nimetusi juurde.",
...
export async function assembleRetrievalContext({
  ...
  const extraSystemInstructions = [
    ...
    ...(municipalityServiceBenefitListRequest ? [buildMunicipalityListInstruction(replyLang)] : []),
      ...
      const primaryRagQueries =
        !sourceLookupRequest && allowMunicipalityScopedRag
          ? buildMunicipalityScopedQueries(ragQueries, effectiveMunicipalities, {
              expandServiceBenefitList: municipalityServiceBenefitListRequest,
