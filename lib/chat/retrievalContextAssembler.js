...
export async function assembleRetrievalContext({
  ...
  if (shouldRunRag) {
    ...
    const legalSelection = buildLegalExactSelection(groupedMatches, legalLookupPlan, {
      ragRiskPolicy
    });
  ...
  return {
    ...
    retrievalMeta: {
      ...
      ragRiskPolicy,
      legalLookupPlan,
      insufficientPreciseLegalSourceSupport: !!(legalLookupPlan?.enabled && legalLookupPlan.mode === "explicit_paragraph" && budgeted.used.length === 0),
      queryPlan,
      hadDocContext: usedDocContext,
