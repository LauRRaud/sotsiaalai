...
import { filterSourcesForReply } from "@/lib/chat/sourceAttribution";
...
export async function handleMainChatResponse({
  ...
  if (!wantStream) {
    try {
      ...
      const replySources = filterSourcesForReply(aiResult.reply, sources, {
  ...
  const sse = new ReadableStream({
    async start(controller) {
      ...
      const finalizeStreamReply = async () => {
        ...
        flushPendingDelta();
        const replySources = filterSourcesForReply(accumulated, sources, {
          query: effectiveMessage
        });
        const { attachments } = await finalizeAssistantReply({
          persist,
