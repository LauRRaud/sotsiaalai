...
function isSubstantiveKnowledgeQuestion(message = "") {
  ...
  const text = normalizeSourceNeedText(message);
  if (!text) return false;
  if (extractExactQueryAnchors(message).length) return true;
...
export function shouldUseExternalSourcesForTurn(message = "", options = {}) {
  ...
  const mentalHealthDigitalSupportDomain =
    /(vaimne tervis|vaimse tervise|psuhho|psĆ¼hho|eneseabi|digitaalne tugi|vestlusrobot|juturobot|chatbot)/.test(text);
  const broadEvidenceQuestion =
    /(kasutus|kasutatakse|kasutab|maini|mainitud|kirjeld|kasitletud|kĆ¤sitletud|esile|kerkinud|valdkond|naide|näide)/.test(text);

  return (socialDomain || mentalHealthDigitalSupportDomain) && (asksForRuleOrProcedure || broadEvidenceQuestion);
}
