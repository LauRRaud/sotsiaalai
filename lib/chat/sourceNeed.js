...
export function shouldUseExternalSourcesForTurn(message = "", options = {}) {
  if (options?.forceSources || options?.sourceLookupRequest) return true;
  if (options?.previousSourceUseRequest) return false;
  if (isAssistantSourceUiIssue(message)) return false;
  if (isLikelyConversationalTurn(message)) return false;
  ...
  const socialDomain =
    /(sotsiaal|hoolekan|hooldus|toet|teenus|abi|puue|puudega|lastekaitse|\bkov\b|omavalitsus|vald|valla|vallas|linn|linna|linnas|taotl|taot|elukoht|rahvastikuregister|\bska\b|sotsiaalkindlustusamet)/.test(text);
  const asksForRuleOrProcedure =
    /(kas|mis|mida|millal|kuidas|kuhu|kellele|kes|milline|tingimus|tingimused|noue|nouded|tahtaeg|summa|maar|maara|kontakt|vorm|saab|voib|peab|tuleb|tohib|kohustus|oigus|menetlus|apply|application|eligible|requirement|deadline|amount|contact|form)/.test(text);
