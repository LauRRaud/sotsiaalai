...
export const ROUTING_POLICY = Object.freeze({
  ...
  [WORK_MODES.GENERAL_QUESTION]: {
    byComplexity: {
      ...
      normal: REASONING_DEPTH.MEDIUM,
      complex: REASONING_DEPTH.MEDIUM
    }
  },
  ...
  [WORK_MODES.CREATE_HELP_REQUEST]: {
    byStep: {
      ...
      refine: REASONING_DEPTH.MEDIUM,
      confirm: REASONING_DEPTH.MEDIUM,
  ...
  [WORK_MODES.DOCUMENT_DRAFTING]: {
    byComplexity: {
      simple: REASONING_DEPTH.MEDIUM,
      normal: REASONING_DEPTH.MEDIUM,
      complex: REASONING_DEPTH.HIGH
    }
  },
  [WORK_MODES.REPORT_DRAFTING]: {
    defaultReasoning: REASONING_DEPTH.HIGH
  }
});
...
export function escalateReasoning(current = REASONING_DEPTH.LOW) {
  if (current === REASONING_DEPTH.LOW) return REASONING_DEPTH.MEDIUM;
  if (current === REASONING_DEPTH.MEDIUM) return REASONING_DEPTH.HIGH;
  return REASONING_DEPTH.HIGH;
}

export function deescalateReasoning(current = REASONING_DEPTH.MEDIUM) {
  if (current === REASONING_DEPTH.HIGH) return REASONING_DEPTH.MEDIUM;
  if (current === REASONING_DEPTH.MEDIUM) return REASONING_DEPTH.LOW;
...
export function shouldEscalateReasoning(input = {}) {
  ...
  if (intent === WORK_MODES.REPORT_DRAFTING) return false;
  if (flags.formalOutput) return true;
  if (flags.manySources) return true;
  if (flags.requestedThoroughness) return true;
  if (intent === WORK_MODES.SERVICE_GUIDANCE && complexity === COMPLEXITY.COMPLEX) return true;
  if (
    (intent === WORK_MODES.CREATE_HELP_REQUEST || intent === WORK_MODES.CREATE_HELP_OFFER) &&
    step === "refine" &&
    flags.manyClarifications
  ) {
    return true;
...
export function chooseReasoningDepth(input = {}) {
  ...
  if (!VALID_REASONING.has(reasoning)) {
    reasoning = REASONING_DEPTH.MEDIUM;
  }
