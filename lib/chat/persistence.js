...
export async function persistInit({
  ...
  try {
    ...
    await prisma.$transaction([prisma.conversationMessage.create({
      data: {
...
export async function persistDone({
  ...
  const operations = [];
  if (finalText) {
    operations.push(prisma.conversationMessage.create({
      data: {
        conversationId: convId,
        role: "ASSISTANT",
        content: finalText,
        metadata: sources?.length || typeof isCrisis !== "undefined" ? {
          sources: sources ?? [],
          isCrisis: !!isCrisis
        } : null
      }
  ...
  operations.push(prisma.conversation.update({
    where: {
      id: convId
    },
    data: {
