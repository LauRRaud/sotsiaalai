...
export function buildResourceDiscoveryQueries(ragQueries = [], baseQuery = "") {
  const normalizedQueries = (Array.isArray(ragQueries) ? ragQueries : [ragQueries])
    .filter(query => typeof query === "string" || (query && typeof query === "object"));
  const base = String(baseQuery || "").trim();
  const discoveryQueries = [
    base,
    `${base} organisatsioonid kontaktid teenusepakkujad tugivorgustik materjalid juhendid PDF`,
    `${base} organization_profile organization_page information_material official_guideline training_material`
  ]
    .map(value => String(value || "").trim())
    .filter(Boolean);

  return Array.from(new Set([...normalizedQueries, ...discoveryQueries]));
}
    ...
    legalLookupPlan.enabled
      ...
      : !sourceLookupRequest && (nationalServiceBenefitQuestion || serviceJurisdictionQuestion)
        ...
        : effectiveResourceDiscoveryQuestion
          ? buildResourceDiscoveryFilters(audienceFilter)
