...
function expandTopicWordCandidates(word = "") {
  const normalized = normalizePlanningText(word);
  if (!normalized) return [];
  const out = [normalized];
  if (normalized.length >= 4 && normalized.length <= 5 && normalized.endsWith("i")) {
    out.unshift(normalized.slice(0, -1));
  }
...
export function extractTopicHints(text = "", maxHints = 6) {
  ...
  const words = normalized
    ...
    .map(word => word.trim())
    .filter(Boolean)
    .flatMap(expandTopicWordCandidates)
...
export function buildTemporalFillQueries({
  ...
  return normalizedYears.flatMap(year => {
    ...
    return Array.from(new Set([
      ...
      ["Eesti sotsiaalvaldkond", yearText].filter(Boolean).join(" ").trim(),
      ["Eesti sotsiaalvaldkond", "sotsiaalkaitse", "sotsiaalhoolekanne", hintText, yearText].filter(Boolean).join(" ").trim(),
      ["Sotsiaaltoo", "sotsiaalpoliitika", hintText, yearText].filter(Boolean).join(" ").trim(),
      ["Sotsiaaltöö", yearText].filter(Boolean).join(" ").trim()
