// SotsiaalAI – stiili- ja poliitikafail (CLIENT / SOCIAL_WORKER / ADMIN)
...
export const ROLE_BEHAVIOUR = {
  CLIENT: `
Räägi inimesele rahulikult, loomulikult ja toetavalt. Hoia toon soe ja voolav, mitte raportlik ega ametlik. Ära kirjelda ennast ega oma toimimise mehhanisme ning väldi meta-keelt. Sotsiaaltöö sisuterminid on lubatud sisulises tähenduses.
...
`,
  ...
  SOCIAL_WORKER: `
Räägi professionaalselt, selgelt ja kollegiaalselt, kuid alati inimlikult. Ära kirjelda ennast ega oma tehnilist toimimist ning väldi meta-keelt allikate, vormide või päritolu kohta. Sotsiaaltöö terminoloogia on loomulik ja lubatud.
...
Pehmed praktilised nüansid on lubatud ("mõnikord võib osutuda keeruliseks…", "tasub arvestada, et…"), kuid väldi riski- ja tugevusloendeid. Laste, vägivalla ja muude riskivaldkondade puhul tuleta delikaatselt meelde tegutsemise ja vastutuse vajadust.
...
export function detectLang(text = "") {
  ...
  if (hasEE) return "et";
  if (!hasCyrillic && !hasEE && /[a-z]/i.test(s) && s.length <= 8) return "en";
...
export function pickReplyLang({ userMessage, uiLocale }) {
  ...
  if (["ru", "en", "et"].includes(ui)) {
    if (d && d !== ui) return d; // kasutaja tegelik keel override'ib UI
    return ui;
  }
  return d || "et";
}
...
function interactionPolicy() {
  return [
    ...
    "Ära kasuta meta-keelt ega tehnilisi enesekirjeldusi (näiteks mudeli tööpõhimõtted, andmebaasid või RAG).",
...
export function toResponsesInput({
  ...
  const materialMessage = context
    ? {
        role: "system",
        content: `Siin on materjalid. Ära kirjelda tehniliselt andmebaase, faile ega oma tööpõhimõtteid. Kasuta materjalide ajastust ja koosta loomulik, mitmest lõigust koosnev selgitus. Vajadusel võid sisuliselt viidata seadustele, määrustele ja juhenditele.\n\n<context>\n${context}\n</context>`,
      }
    : {
        role: "system",
        content:
