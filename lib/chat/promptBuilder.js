...
function buildGroundingMessage({ grounding, context, replyLang = "et" } = {}) {
  ...
  if (normalizedReplyLang === "en") {
    return {
      ...
      content: [
        "RAG_GROUNDING: weak.",
        "The retrieved RAG context is partial or uneven.",
        "Do not present the answer as a complete overview of the whole field.",
        "Frame conclusions as based on the available RAG context.",
        "Do not add years, policy changes, legal details, amounts, deadlines, or official requirements that are not visible in RAG_CONTEXT.",
        "If the user asks for the main changes, distinguish that these are main items only within the visible sources."
      ].join(" ")
    };
  }
  ...
  if (normalizedReplyLang === "ru") {
    return {
      ...
      content: [
        "RAG_GROUNDING: weak.",
        "Найденный RAG-контекст неполный или неровный.",
        "Не представляй ответ как полный обзор всей области.",
        "Формулируй выводы как основанные на доступном RAG-контексте.",
  ...
  return {
    ...
    content: [
      "RAG_GROUNDING: weak.",
      "Leitud RAG-kontekst on osaline või ebaühtlane.",
      "Ära esita vastust täieliku ülevaatena kogu valdkonnast.",
      "Sõnasta järeldused kättesaadava RAG-konteksti põhjal.",
      "Ära lisa aastaid, poliitikamuudatusi, õiguslikke detaile, summasid, tähtaegu ega ametlikke nõudeid, mida RAG_CONTEXT ei näita.",
      "Kui kasutaja küsib peamisi muudatusi, erista, et need on peamised ainult nähtud allikate piires."
    ].join(" ")
  };
}
