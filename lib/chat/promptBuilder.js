...
Hoia sisu võimalikult lähedal tekstis endas olevale tasemele: võid mõtet ümber sõnastada selgemalt või lihtsamalt, kuid ära lisa uusi üldistusi, väärtushinnanguid ega vastandusi (näiteks “abi vs sundus”, “ärilised eesmärgid vs toetus”, “ettevõtlikkus vs sotsiaaltöö”), kui neid ei ole tekstis selgelt välja öeldud. Ära loo uusi teoreetilisi silte või rolle (näiteks “sotsiaalne algatus”, “juhtimisfilosoofia”, “kaasav töökohtavalik”), kui neid sõnu materjalis ei kasutata. Ära leiuta juurde kujundeid, mis vastandavad kirjeldatut hoolekandeasutusele, „varjupaigale” või “päris teenuskohale”, kui sellist vastandust tekstis ei ole. Võid ühendada sama teksti eri lõikudes esinevaid fakte ja mõtteid lühikeseks sidusaks kokkuvõtteks, kui see ei lisa uusi fakte ega väärtushinnanguid.
...
Hoia sisu võimalikult lähedal materjalis olevale. Võid siduda erinevaid lõike ja rõhuasetusi, kuid ära loo uusi abstraktseid dilemmakirjeldusi või väärtusvastandusi (näiteks “ärilised eesmärgid vs toetus”, “ettevõtlikkus vs sotsiaaltöö”, “abi peale surumine”), kui selliseid vastandusi tekst ise ei esita. Ära pane asjadele uusi teoreetilisi nimesid, mida materjalis ei esine (näiteks “sotsiaalne algatus”, “juhtimisfilosoofia”, “kaasav töökohtavalik”), ega vastanda kirjeldatut hoolekandeasutusele, “varjupaigale” või “päris teenuskohale”, kui sellist vastandust tekst ise ei loo. Võid ühendada materjalis eri lõikudes esinevaid fakte ja mõtteid sidusateks kokkuvõtlikeks lõikudeks, kui need ei lisa uusi väiteid ega hinnanguid.
...
export function pickReplyLang({ userMessage, uiLocale }) {
  ...
  // Kui UI on selgelt et/ru, siis ei lükata vastust inglise keelele pelgalt heuristika alusel
  if (ui === "et" || ui === "ru") {
  ...
  // kui UI keelt pole või on muu, proovime tuvastust; vaikimisi eesti
  return d || "et";
}
...
function groundingPolicy() {
  return [
    ...
    // lubatud siduv kokkuvõte
    "Võid siduda materjalides eri kohtades esinevaid fakte ja mõtteid lühikesteks sidusateks kokkuvõteteks, kui need ei lisa uusi väiteid, põhimõtteid ega hinnanguid.",

    // ei leiuta uusi teooriaid ega väärtusvastandusi
    "Kui kasutad mõnd teoreetilist mõistet, raamistust või põhimõtet, peab see esinema vähemalt ühes ette antud materjalis; ära loo uusi teooriaid, kategooriaid ega väärtusvastandusi (näiteks vastandused teise tüüpi asutustele või lähenemistele), mida materjal ise ei kasuta.",

    // üldistuste ja mudeliloome keeld
    "Ära tee materjali põhjal laiu üldistusi ega järeldusi kujul 'sellised lahendused toimivad alati…' või 'sotsiaaltöö vaatenurgast tähendab see, et…', kui materjal ise sellist üldistust ei esita; jää konkreetse kirjelduse tasandile ega muuda üksikjuhtumit üldiseks mudeliks või strateegiaks.",

    // eri materjalide seosed
    "Kui eri materjalides on sama teema kohta erinevaid rõhuasetusi või infot, võid neid neutraalselt kõrvutada, kuid ära leiuta nende vahele seost, mida tekstid ise ei loo.",

    // ajakonteksti rangus
    "Kui materjalis on kirjas konkreetne aastaarv või ajavahemik, kasuta sama ajakonteksti kirjeldustes (näiteks 'see kirjeldus käib 2017. aasta kohta'), ilma uusi ajamääratlusi juurde mõtlemata.",
    "Kui tekstis kasutatakse suhtelisi ajaväljendeid (näiteks 'eelmisel suvel', 'toona'), võid neid vastuses kasutada samas vormis, kuid ära tuleta neile uusi aastaarve, kui need ei ole tekstis selgelt kirjas.",
    "Kui materjalis on kuupäevad või summad, võid neid mainida koos ajaviitega.",

    // innovatsiooni reegel
    "Kui kasutaja palub sõnaselgelt luua uue idee, teenuse, mudeli või kontseptsiooni (näiteks 'mõtle välja', 'paku uus lahendus', 'leiuta midagi sellise lähenemise põhjal'), võid pakkuda loomingulisi ja innovaatilisi ettepanekuid. Sel juhul ütle lühidalt, et järgnevad on sinu loodud ideed, mitte materjalides toodud faktid, ning ära esita neid olemasoleva olukorra kirjelduse pähe.",

    // stiil: mitte meta-tehniline
