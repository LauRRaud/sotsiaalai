...
function styleRule(effectiveRole) {
  if (effectiveRole === "SOCIAL_WORKER") {
    return [
      ...
      "For source-grounded specialist questions, do not give a bare yes/no answer followed mainly by an offer to continue. Give a concise substantive answer in the first turn with 3-5 concrete details from the source and the source scope or limitation.",
...
export function buildResponsesPayload(input, options = {}) {
  const verbosity = options.verbosity || (options.effectiveRole === "SOCIAL_WORKER" ? "medium" : "low");
  const payload = {
    ...input,
