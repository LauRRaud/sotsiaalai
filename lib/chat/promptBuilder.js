...
function isSimpleAvailabilityQuestion(text = "") {
  ...
  return (
    (
      /\b(kas sul on|kas teil on|on sul|on teil)\b/.test(s) &&
      /\b(seadus|maarus|maarus|juhend|dokument|allikas|materjal|teema|vorm|blankett|teenus|toetus)\b/.test(s)
    ) ||
    /\b(kas .* olemas)\b/.test(s) ||
    /\b(do you have|is there|is .* available)\b/.test(s)
  );
}
...
function preferredVerbosityForTurn(history = [], userMessage = "") {
  if (
    isSimpleAvailabilityQuestion(userMessage) ||
    isInternalCauseQuestion(userMessage) ||
    isFollowupWhyAfterSelfCorrection(history, userMessage)
  ) {
    return "low";
  }
  return null;
...
function turnSpecificRule(history = [], userMessage = "", replyLang = "et") {
  ...
  if (isSimpleAvailabilityQuestion(userMessage)) {
    rules.push([
      "TURN_INSTRUCTION:",
  ...
  if (isInternalCauseQuestion(userMessage)) {
    rules.push([
      "TURN_INSTRUCTION:",
  ...
  if (isFollowupWhyAfterSelfCorrection(history, userMessage)) {
    rules.push([
      "TURN_INSTRUCTION:",
  ...
  if (historyHasUnexpectedCyrillic(history, replyLang)) {
    rules.push([
      "TURN_INSTRUCTION:",
...
export function buildResponsesPayload(input, options = {}) {
  const verbosity = options.verbosity || input.preferredVerbosity || "medium";
  const payload = {
    ...input,
