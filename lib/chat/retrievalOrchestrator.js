...
export function isBroadMultiSourceRagQuestion(message = "") {
  ...
  return broadIntent && multiSourceScope;
}
...
export function buildSourceLookupSearchQuery(message = "", history = []) {
  ...
  if (isToimetulekutoetusLegalSectionLookup(combined)) {
    parts.push(...buildToimetulekutoetusLegalSectionAnchors(subject || "Sotsiaalhoolekande seadus"));
  }
