...
export function buildDocumentIntakeReply({
  ...
  return [
    "Sain aru: see on dokumendiulesanne ja saan kaivitada sama agent-too voo nagu dokumendi koostamise vaates.",
    "",
...
export function buildDocumentMissingSourcesReply(replyLang) {
  ...
  return "Mustandit ei saa veel kaivitada, sest agendile lubatud allikaid ei ole. Lisa failid Dokumentidesse ja luba need dokumendi koostamise jaoks.";
}
...
function isDocumentCancel(message = "") {
  return /\b(katkesta|alusta otsast|lopeta|lõpeta|cancel|abort)\b/.test(normalizeIntentText(message));
}
