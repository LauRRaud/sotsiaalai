// Room message pub/sub with Redis fallback to in-memory.
import crypto from "node:crypto";
import { getRedisClients } from "./redis";

const subscribers = new Map(); // roomId -> Set<fn>
const nodeId = crypto.randomUUID();
const CHANNEL = "room-events";

const { pub: redisPub, sub: redisSub } = getRedisClients();
if (redisSub) {
  redisSub.on("message", (_channel, payload) => {
    try {
      const { roomId, event, node } = JSON.parse(payload);
...
export function publishRoomEvent(roomId, event) {
  emitLocal(roomId, event);
  if (redisPub) {
    redisPub.publish(CHANNEL, JSON.stringify({ roomId, event, node: nodeId })).catch(() => {});
  }
}
