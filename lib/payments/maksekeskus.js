...
const MAKSEKESKUS_RECURRING_API_BASE = String(
  process.env.MAKSEKESKUS_RECURRING_API_BASE || MAKSEKESKUS_API_BASE || ""
)
  .trim()
  .replace(/\/+$/, "");
...
export async function createMaksekeskusRecurringCharge({
  ...
  if (!MAKSEKESKUS_RECURRING_API_BASE || !MAKSEKESKUS_API_KEY || !MAKSEKESKUS_SHOP_ID) {
  ...
  try {
    const response = await fetch(`${MAKSEKESKUS_RECURRING_API_BASE}/payments/recurring`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${MAKSEKESKUS_API_KEY}`
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok) {
      const err = new Error(data?.messageKey || data?.message || "api.subscription.recurring_charge_failed");
      err.status = response.status;
      err.payload = data;
      throw err;
    }
