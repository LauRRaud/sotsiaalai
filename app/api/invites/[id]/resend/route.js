...
export async function POST(_req, { params }) {
  ...
  try {
    ...
    if (invite.status !== "SENT") {
      return json({ ok: false, message: "Only pending invites can be resent" }, 400);
    }
    ...
    if (invite.expiresAt <= now) {
      return json({ ok: false, message: "Invite expired" }, 410);
    }

    const { raw, hash } = randomToken();
    await prisma.invite.update({
      where: { id },
      data: { tokenHash: hash, status: "SENT" },
    });
    ...
    return json({ ok: false, message: "Resend failed" }, 500);
  }
}
