...
export async function POST(request) {
  try {
    ...
    if (!loginToken || loginToken.usedAt || loginToken.otpVerifiedAt || !loginToken.requiresOtp || loginToken.expiresAt <= new Date()) {
      return json({
        message: "Seda koodi ei saa uuesti saata.",
        code: "TOKEN_INVALID"
      }, 400);
    }
    ...
    const {
      expiresAt
    } = await resendOtp(user.email, loginToken.userId);
    return json({
      status: "resent",
    ...
    return json({
      message: "Koodi saatmine ebaõnnestus."
    }, 500);
  }
}