...
export async function DELETE(_req, { params }) {
  ...
  if (!roomId || !msgId) return json({ ok: false, message: "Missing roomId or msgId" }, 400);
  ...
  try {
    ...
    try {
      publishRoomEvent(roomId, { type: "delete", id: msgId });
    } catch {}

    return json({ ok: true, id: msgId, deleted: true });
  } catch (err) {
    console.error("[room message delete] failed", err);
    return json({ ok: false, message: "Kustutamine ebaõnnestus" }, 500);
  }
}
