...
export async function GET(_req, {
  ...
  const [room, members] = await Promise.all([prisma.room.findUnique({
    where: {
      id: roomId
    },
    select: {
  ...
  return json({
    ...
    members: members.map(m => ({
      userId: m.userId,
      role: m.role,
      name: m.displayName || [m.user.profile?.firstName, m.user.profile?.lastName].filter(Boolean).join(" ") || "Liige",
