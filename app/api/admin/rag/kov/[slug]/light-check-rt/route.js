import { runKovRtLightCheckBySlug } from "@/lib/admin/rag/kov/service";
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    console.error("[kov-admin] rt light check failed", { slug, error: safeError(error) });
    return json(
      {
        ok: false,
        message: String(error?.message || "KOV RT light check failed")
      },
      status
    );
  }
