import { runKovLightChecksBySlugs } from "@/lib/admin/rag/kov/service";
...
export async function POST(request) {
  ...
  try {
    const items = await runKovLightChecksBySlugs(slugs);
    return json({
      ok: true,
      items,
      count: items.length
    ...
    console.error("[kov-admin] bulk light check failed", safeError(error));
