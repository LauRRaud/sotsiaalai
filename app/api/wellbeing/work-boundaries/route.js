...
export async function POST(request) {
  ...
  try {
    ...
    const record = await createWorkBoundariesRecordForUser(auth.userId, body);
    return wellbeingJson({ ok: true, record }, 201);
  } catch (error) {
    const status = Number(error?.status) || 500;
    if (status >= 500) {
      console.error("[wellbeing] work-boundaries save failed", safeError(error));
    }
    return wellbeingJson({
      ok: false,
      message: error?.message || "wellbeing.errors.work_boundaries_save_failed",
