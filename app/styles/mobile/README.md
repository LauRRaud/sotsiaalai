...
- Profile orbit/dock mobile rules moved to `app/styles/features/profile/mobile.css`; the profile vertical (mono + hc + mobile) is route-imported via `app/styles/features/profile/index.css` on /profiil and /vestlus. NB: `home-scroll.css` still carries a few unscoped profile-orbit-stack rules (see its entry).
