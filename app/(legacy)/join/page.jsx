...
export default function JoinPage() {
  ...
  if (!token) {
    return <main id="main" className="main-content glass-box profile-container">
        <h1 className="glass-title">
          {t("join.missing_title", "Kutse link puudub")}
        </h1>
        <p>
  ...
  return <main id="main" className="main-content glass-box profile-container">
      ...
      {status !== "authenticated" ? <div className="glass-section">
          <p>
            {t("join.signin_prompt", "Logi sisse või loo konto, et liituda.")}
          </p>
          <Button type="button" onClick={() => setLoginOpen(true)}>
            {t("join.signin", "Logi sisse")}
          </Button>
        </div> : <div className="glass-section">
          <p>
          ...
          <Input id="join-display-name" value={displayName} onChange={e => setDisplayName(e.target.value)} disabled={busy} />
          <Button type="button" onClick={accept} disabled={busy}>
            {busy ? t("join.joining", "Liidan...") : t("join.join_button", "Liitu")}
          </Button>
        </div>}

      {statusMsg ? <div className="glass-note glass-note--success" role="status">
          {statusMsg}
        </div> : null}
      {error ? <div className="glass-note glass-note--error" role="alert">
          {error}
        </div> : null}
