...
export default function HomePage() {
  // Vasak kaart — kaks magnetit (front/back)
  const magnetLeftFront = useRef(null);
  const magnetLeftBack = useRef(null);
  ...
  // Parem kaart — kaks magnetit (front/back)
  const magnetRightFront = useRef(null);
  const magnetRightBack = useRef(null);
  ...
  function animateLeft() {
    ...
    // Nähtaval poolel liigub kuma kaasa!
    const activeMagnet = isLeftFlipped ? magnetLeftBack.current : magnetLeftFront.current;
    if (activeMagnet) {
      activeMagnet.style.transform = `translate(${coordsLeft.current.x}px, ${coordsLeft.current.y}px)`;
    }
  ...
  function animateRight() {
    ...
    const activeMagnet = isRightFlipped ? magnetRightBack.current : magnetRightFront.current;
    if (activeMagnet) {
      activeMagnet.style.transform = `translate(${coordsRight.current.x}px, ${coordsRight.current.y}px)`;
    }
  ...
  function handleMouseMoveLeft(e) {
    ...
    if (dist < 340) {
      ...
      if (isLeftFlipped) {
        magnetLeftBack.current?.classList.add("magnet-active");
        magnetLeftFront.current?.classList.remove("magnet-active");
      } else {
        magnetLeftFront.current?.classList.add("magnet-active");
        magnetLeftBack.current?.classList.remove("magnet-active");
      }
    } else {
  ...
  function handleMouseMoveRight(e) {
    ...
    if (dist < 340) {
      ...
      if (isRightFlipped) {
        magnetRightBack.current?.classList.add("magnet-active");
        magnetRightFront.current?.classList.remove("magnet-active");
      } else {
        magnetRightFront.current?.classList.add("magnet-active");
        magnetRightBack.current?.classList.remove("magnet-active");
      }
    } else {
  ...
  // Kaardi pööramise tuvastus
  function handleLeftFlip(e) {
    // Kui on hover või fookus, siis tagakülg
    setIsLeftFlipped(
      e?.type === "mouseleave"
        ? false
        : e?.currentTarget?.matches(":hover, :focus-within")
    );
  }
      ...
      <div className="main-content">
        ...
        >
          <div
            ...
            ref={cardLeft}
            onMouseEnter={handleLeftFlip}
            onMouseLeave={handleLeftFlip}
            onFocus={handleLeftFlip}
            onBlur={handleLeftFlip}
          >
        ...
        >
          <div
            ...
            ref={cardRight}
            onMouseEnter={handleRightFlip}
            onMouseLeave={handleRightFlip}
            onFocus={handleRightFlip}
            onBlur={handleRightFlip}
          >
            <div className="card-wrapper">
              <div className="card-face front">
                <div
                  className="magnet-wrapper"
                  ref={magnetRightFront}
                  style={{
                    transition: "none",
                    willChange: "transform"
                  }}
