...
export function isMunicipalityType(value) {
  return MUNICIPALITY_TYPE_VALUES.includes(String(value || "").trim().toUpperCase());
}

export async function getMunicipalityById(municipalityId, prismaClient = prisma) {
...
export async function listMunicipalities(filters = {}, prismaClient = prisma) {
  const query = String(filters?.query || "").trim();
  const county = String(filters?.county || "").trim();
  const type = String(filters?.type || filters?.kind || "").trim().toUpperCase();
  const where = {};
  ...
  if (query) {
    where.OR = [
      ...
      {
        slug: {
          contains: normalizeMunicipalitySearchText(query),
          mode: "insensitive"
        }
      }
    ];
  ...
  return prismaClient.municipality.findMany({
    where,
    select: municipalitySummarySelect,
    orderBy: [{ county: "asc" }, { displayName: "asc" }],
    take: normalizeLimit(filters?.limit, 50, 100)
  });
}
