...
1c. **`reports/css-important-reduction-method.md` (2026-06-15)** — `!important` vähendamise meetod + KASKAADI-KIHI REEGEL (Tailwindi `!` kihis võidab käsitsi-`!important`-i; CSS-`!important` = teema-sõda mitte Tailwind; teemad `!important`-vabalt token-mustriga) + tööriist `scripts/css-important-overrides.mjs`.
...
- **CSS SAFE-LOOP süsteem ehitatud (15.06.2026).** `scripts/css-cleanup/` — ise-verifitseeruv ahel olemasoleva snapshot/page-report infra peal. **Loe esmalt: `scripts/css-cleanup/README.md`.** Komponendid: `targets-gen.mjs` (täielik katvus page-reportist → `css-snapshot.targets.generated.json`, 48 targetit), `worklist-gen.mjs` (võla-nimekiri failide kaupa, top = `theme/hc.css` 328), `run.mjs` (orkestraator: `before --file X` → muuda → `verify --file X [--auto-revert]`; gate1 snapshot-diff + gate2 npm test). `css-snapshot.mjs` täiendatud: `--all` (kõik instantsid) + `waitFor`-samm. **Kaks turvaluku:** (1) täielik katvus — vana 7-selektori target oli illusioon; (2) **ALATI headed** — headless ei renderda token/glass/canvas komponente õigesti, mõõdaks vale asja (before/after mõlemad valed → diff valeroheline). **LIVE-PoC + 2 ise-testi VALMIS (15.06.2026, auth-vabalt /hinnastusel):** tühi-jooks=🟢 identical, tahtlik regressioon=🔴 püütud kõigis teemades. **Flaky-parandus:** ise-test paljastas, et BorderGlow box-shadow emiteerib täiesti läbipaistvaid (`rgba(...,0)`) glow-kihte, mille geomeetria jitterib jooksude vahel → valepositiivsed. `css-snapshot-diff.mjs` ignoreerib nüüd läbipaistvaid varjukihte (nähtav kiht jääb; alpha>0 muutus püütakse ikka). **Fidelity-parandus (KRIITILINE, 15.06.2026):** kasutaja märkas, et mid-teema renderdus jäädvustamisel valesti (mono taust mid CSS-aktsentidega). Juur: app loeb teema TÕEALLIKAT `a11y_prefs` **küpsisest** (server `layout.js parseA11yPrefs` → React-kontekst + `BackgroundLayer`; klient `AccessibilityProvider.readPrefsFromCookie`). `css-snapshot.mjs` seadis ainult localStorage → CSS-klassid õiged, aga React/JS-joonistatud pinnad (taust, glow) jäid default "mono" peale. **Fix:** `applyTheme` seab nüüd ka `a11y_prefs` küpsise (`encodeURIComponent(JSON)`, sama kodeering nagu app `setCookie`). Visuaalselt kinnitatud (mid = pruun). + `captureTarget` austab nüüd `target.themes` (ignoreeris seda → jooksis kõik 6). **ÕPPETUND:** snapshot-tööriista teema/seisu seadmine peab matchima rakenduse PÄRIS-mehhanismi (küpsis+localStorage+server), mitte ainult ühte kihti. **Faithful baseline tehtud (15.06.2026):** hc.css 187 selektorit × hc+light × 46 route'i → `state/ea056487ddd2.before.json` (57MB, gitignore). Katvus **115/187** (72 modal/olek-gated = Tier 2 vajab flow'sid). Capture-resilientsus lisatud (redirektiv /teekond, /rooms → null + jätka, ei kukuta crawli). **Live tokeniseerimis-demo (chat-send-btn rgba(255,234,0,X)→rgba(var(--hc-accent-rgb),X)) õpetas 2 asja:** (1) **värvi-serialiseerimine pole stabiilne** — literaal `rgba()` → hex `#ffea00a8`, aga `rgba(var(),..)` jääb rgba; ilma normaliseerimata ei saa tokeniseerimist KUNAGI rohelisena verifitseerida → `css-snapshot-diff.mjs` kanoniseerib nüüd kõik värvid #rrggbbaa-ks (alpha baidina). Pärast seda: tokeniseerimise diffid kadusid (tõestatult ekvivalentne). (2) **stateful lehed (/vestlus) pole golden-master-itavad** — DOM erineb laadimiste vahel (glass-ring paneel, back-icon hover) → jääkmüra. **Õppetund: broad-tokeniseerimine verifitseeri STAATILISTEL lehtedel.** **Veel tegemata:** reaalne hc.css tokeniseerimine (204 hardcoded `rgba(255,234,0,X)` → kanali-token), verifitseeritud staatiliste route'ide diffiga; + Tier 2 modal-flow'd 72 katmata selektorile.

## KAMPAANIA 1 (teema-kiht tokeniseerimine) + !important-MEETOD (15.06.2026, Opus)

- **Kampaania 1 teema-kiht VALMIS:** kõik 6 teemafaili + mono features (chat/documents/profile) + HC features (chat/profile) tokeniseeritud → kanali-tokenid (`--hc-accent-rgb`, `--mono-light/mid-rgb`, `--dark-white/black-rgb`, `--mid/light/night-white-rgb`, `--night-blue-rgb`). Commits `e369e995`→`32ab6cac`, `48b09987`. **Gate 1 (visuaalne) JA gate 2 (npm test) verifitseeritud.**
- **GATE-2 ÕPPETUND:** tokeniseerimine lõhub teksti-kontrakt-teste (grep'ivad literaal-värvi lähtekoodist). ALATI jooksuta `npm test` (mitte ainult `--no-tests`). Loader bundle'ib theme+tokens+chat+profile hc.css → uuenda KÕIK hc-bundle assertid token-vormi, jäta literaaliks need mis loevad tokeniseerimata faile. Parandused `490f5a0f`, `48b09987`. Testibaseline tagasi 12 (`026c5791` parandas ka pre-existing mobile-topnav stale asserti). Vt `reports/css-campaign1-progress-2026-06-15.md`.
- **CRLF:** Edit-tööriist Windowsil kirjutab CRLF; `.gitattributes` nõuab `*.css eol=lf` → konverteeri working tree LF-iks (muidu mitmerealine `\n` indexOf assert kukub).
- **!important-MEETOD + KASKAADI-KIHI REEGEL + TÖÖRIIST:** vt **`reports/css-important-reduction-method.md`** (kandub kontovahetust üle). Lühike: Tailwindi `!` (kihis) võidab käsitsi-`!important`-i → CSS-`!important` = teema-sõda mitte Tailwind; teemad saab `!important`-vabalt token-mustriga; tööriist `scripts/css-important-overrides.mjs` (`2c625e1d`) leiab võitja + mida üle kirjutab + verdikt + eemaldamis-kandidaadid (gate-guarded). Mõõtmine: mid.css `!important` on valdavalt KANDEV → vaja token-migratsiooni, mitte kustutamist.
