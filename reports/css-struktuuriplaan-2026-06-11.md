...
### Etapp 6c — kaskaadi-konsolideerimine (night), tehtud 12.06.2026 (commitid 5b6485ef, c345cf40)
- Käidi läbi kogu dark.css `:not()`-ahel. Leiti veel **2 baidi-identset night-koopiat** (lisaks piloodi invite-glow + help-listings'ile):
  - **login-modal--otp ×3** (login-modal-shell / fancy-checkbox--otp / .box): dark.css selektoritest eemaldatud `:not(.theme-night)`, night.css 3 koopiat kustutatud. Kehad on `var()`-põhised → night loeb endiselt oma `--glass-modal-bg` (= `--glass-ring-surface-bg`, rgba(16,21,31,0.41)); mid/light jäid välistatuks (oma erinevad koopiad). **dup 1388 → 1381** (üks kloonipaar kadus, 107 → 106 klooni).
  - **framework-page-shell** `--framework-panel-shadow: none`: sama operatsioon. **dup püsis 1381** — 1-realine reegel on alla jscpd minToken-läve, seega jscpd ei loenda seda kloonina; konsolideerimine on siiski päris baidi-identne eemaldus (käsitsi + brauseris tõestatud). Õppetund: jscpd "dup peab langema" check kehtib ainult ≥5-realistele reeglitele; väiksemate puhul on baidi-identsuse + arvutatud-stiili kontroll ainus valvur.
- Mõlemal sammul: testid = samad 12 baseline-kukkumist (0 regressiooni); brauseriproov arvutatud stiilidel night vs default-dark vs mid/light — night saab oma õiged väärtused konsolideeritud reegli kaudu, mid/light jäävad muutmata.
- **Dark.css `:not()`-ahel on nüüd ammendatud baidi-identsete night-koopiate osas.** Ülejäänud välistatud-teema reeglid on tõeliselt erineva kehaga (big var-block, materials-page-shell sinine/mono palett, invite-list :is() border:0+backdrop) VÕI koopiat pole (invite-refresh-btn, materials-upload buttons). Kõrvalleid: `--workspace-elevated-card-bg` definitsioon on dark/night vahel baidi-identne, AGA elab per-teema palett-blokkide sees (ümbritsevad deklaratsioonid erinevad) → pole `:not()`-token-drop kandidaat, vajaks eraldi shared-reegli ekstraktsiooni (riskantne: var viitab teemapõhisele `--glass-ring-surface-bg`-le).

### LeftRail ↔ RightRail jagatud moodul, tehtud 12.06.2026 (commitid ee049156, 9670cfc4, 85783459, 8854718f)
- Railid olid struktuurselt peaaegu kaksikud (~250 dubleeritud rida). Loodi `components/chat/rail.module.css` ja kõik küljest sõltumatu viidi sinna **CSS-mooduli `composes` kaudu** (esmakordne muster repos; valideeritud dev-buildiga). JSX ei muutunud (`styles.X` annab endiselt töötava komposiitklassi).
  - **step 0** (ee049156): slot/pointerBlocked/mobileRailTransition/tooltipAnchor/isActive — composes-mustri valideerimine.
  - **core** (9670cfc4): `.item` baas (kumbki rail hoiab ainult oma `left`), iconBtn + HC-reset + iconSvg/iconChats/iconRooms.
  - **tooltip** (85783459): tooltip-baas + KÕIK teema-variandid (light/mid/night/default-dark/mono/hc + @supports + ui-profile). Suurim tükk. Teemareeglite suhteline järjekord säilitatud (default-dark enne mono; HC-border `!important` 2px enne HC-värvi) → kaskaad identne.
  - **container** (8854718f): `.rail` konteiner + ui-profile + modal-open jagatud; kumbki rail lisab ainult `left`/`right` + külje `width`-var.
- **jscpd components/chat dup 249 → 9 rida** (10 → 1 kloon). Kontraktitestid (hc/mono) suunatud lugema `rail.module.css`-i (hc-reset + mono-tooltip elavad nüüd seal üks kord). Kõik 34 rail/orbiit kontrakti läbivad.
- **Õppetunnid:** (a) `composes` TÖÖTAB selles Next.js-buildis; element kannab nii lokaalset kui jagatud hashi, jagatud `:global(...) .X` reeglid haakuvad. (b) Composes-järjekord: jagatud baas emiteeritakse enne lokaalset → lokaalne `.tooltipVisible{opacity:1}` ületab jagatud `.tooltip{opacity:0}` (brauseris kinnitatud). (c) Side-spetsiifiline jääb lokaalseks: positsioneerimine, tooltipVisible/Hidden (transform-i märk), Left mono back-icon, Right ::before/.label. (d) Tagasinupu tooltip-summutus on EELNEV JSX-disain (`if (item.key === "back") return` ×4), mitte CSS — jäeti nii.
- **OrbitalMenu "531 dup-rida" oli FANTOOM:** jscpd 4.2.5 leiab failisisest dup'i ainult ~46 rida; 531 tuli ilmselt globaalsest jooksust vs `safety_snapshots/orbital_dim_2026-03-10/` varukoopia. Profiili-leht kasutab OrbitalMenu'd (`profile-orbit-*`/`dock-*`, tavaline CSS, mitte moodul) — eraldi komponent railidest, visuaalne keel sarnane aga kood jagamata.
...
- **Väikesed eraldi tööd:** ~~LeftRail↔RightRail ühendamine~~ TEHTUD (vt ülal, dup 249→9). ~~OrbitalMenu.css failisisene dedup (531 rida)~~ FANTOOM (päris ~46 rida). Veel: OrbitalMenu.css surnud-koodi audit (2018 rida, käib); tombstone'ide kustutamine pärast testide ümbersuunamist (chat-focus.css, documents-mode.css, glass.css ja helpers-core.css aggregaatorid, a11y.css); invite/help-listings/selected-listing shared-otsus.
