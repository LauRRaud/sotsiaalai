...
export function useChatStream(config) {
  ...
  const sendMessage = useCallback(async rawText => {
    ...
    if (cfg.isRoomMode) {
      if (cfg.roomBlocked) {
        cfg.setErrorBanner?.(cfg.t?.("chat.room.blocked", "Vestluses osalemine ei ole hetkel voimalik. Palun vota uhendust oma spetsialistiga.") ?? "Vestluses osalemine ei ole hetkel voimalik.");
        return false;
      }
      if (cfg.roomAuthRequired) {
        cfg.setErrorBanner?.(cfg.t?.("chat.room.auth_required", "Sessioon aegus. Palun logi uuesti sisse.") ?? "Sessioon aegus.");
        return false;
      }
      try {
        ...
        if (res.status === 403) {
          cfg.setErrorBanner?.(data?.message || (cfg.t?.("chat.room.blocked", "Vestluses osalemine ei ole hetkel voimalik. Palun vota uhendust oma spetsialistiga.") ?? "Vestluses osalemine ei ole hetkel voimalik."));
          return false;
        }
        if (res.status === 401) {
          cfg.setErrorBanner?.(cfg.t?.("chat.room.auth_required", "Sessioon aegus. Palun logi uuesti sisse.") ?? "Sessioon aegus.");
        ...
        if (!res.ok || data?.ok === false) {
          const msg = data?.message || cfg.t?.("chat.room.send_error", "Viga saatmisel") || "Viga saatmisel";
          throw new Error(msg);
        }
        ...
        cfg.setErrorBanner?.(err?.message || cfg.t?.("chat.room.send_error", "Viga saatmisel") || "Viga saatmisel");
        return false;
      }
    }
    ...
    const runStream = async () => {
      try {
        ...
        if (res.status === 429) {
          ...
          throw new Error(retry ? `Liiga palju päringuid. Proovi ~${retry}s pärast.` : "Liiga palju päringuid. Proovi varsti uuesti.");
        }
        ...
        if (!contentType.includes("text/event-stream")) {
          ...
          const replyText = (data?.answer ?? data?.reply) || "Vabandust, ma ei saanud praegu vastust koostada.";
        ...
        for await (const ev of reader) {
          if (ev.event === "meta") {
            try {
              ...
              const rawSources = Array.isArray(payload?.sources) ? payload.sources : Array.isArray(payload?.groups) ? payload.groups : null;
              if (rawSources) {
                const normalize = cfg.normalizeSources || defaultNormalizeSources;
