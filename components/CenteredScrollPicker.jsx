...
export default function CenteredScrollPicker({
  ...
  const setHiddenStateForItem = useCallback(
    (itemEl, hidden) => {
      if (!isHTMLElement(itemEl)) return;

      if (hidden) itemEl.setAttribute("aria-hidden", "true");
      else itemEl.removeAttribute("aria-hidden");
      ...
      if (hidden) {
        ...
        for (const node of focusables) {
          ...
          const prev = store.get(node); // string | null
          if (prev === null || prev === undefined) node.removeAttribute("tabindex");
  ...
  const applyHiddenFocusGuards = useCallback(
    (newActiveIdx) => {
      const items = getItems();
      if (!items.length) return;
      ...
      if (typeof document !== "undefined") {
        ...
        if (isHTMLElement(activeEl)) {
          ...
          if (hiddenAncestor && isHTMLElement(hiddenAncestor)) {
            ...
            const focusables = activeItem
              ? Array.from(activeItem.querySelectorAll(FOCUSABLE_SELECTOR)).filter(isHTMLElement)
              : [];
  ...
  const scheduleSettle = useCallback(() => {
    ...
    settleTimerRef.current = window.setTimeout(() => {
      ...
      if (Math.abs(delta) > maxStepPerSettle) {
        ...
        commitActiveIndex(target, { silent: true });
      }
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      // IMPORTANT:
      // When wheel-lock is enabled, we do programmatic smooth scrolling step-by-step.
      // Running an extra “settle” snap on top causes visible jitter (“tõksub”).
  ...
  // Optional wheel-to-steps lock (prevents skipping)
  useEffect(() => {
    const el = containerRef?.current;
