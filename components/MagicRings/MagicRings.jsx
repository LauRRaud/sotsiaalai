...
void main() {
  ...
  float alpha = intensity < uAlphaCutoff ? 0.0 : intensity * uOpacity;
  gl_FragColor = vec4(c, alpha);
}
`;

function resolveCssColor(value, element) {
  if (typeof value !== "string" || !value.startsWith("var(") || !element) return value;
  const parts = value.slice(4, -1).split(",");
  const propertyName = parts[0]?.trim();
  const fallback = parts.slice(1).join(",").trim();
  if (!propertyName) return fallback || "#ffffff";
  return getComputedStyle(element).getPropertyValue(propertyName).trim() || fallback || "#ffffff";
}
...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    const animate = time => {
      ...
      const color = resolveCssColor(props.color, mount);
      const colorTwo = resolveCssColor(props.colorTwo, mount);
      ...
      uniforms.uAlphaCutoff.value = props.alphaCutoff;
  ...
  return (
    <div
      ref={mountRef}
      className={styles.magicRingsContainer}
