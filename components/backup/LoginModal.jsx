...
function SubmitArcOverlayWhite({ size = 80, filled = 0, max = 8 }) {
  ...
  const clamped = Math.min(max, filled);
  const eased = easeOutCubic(clamped / max);

  // Small overshoot to visually reach the arc end with round linecaps
  const EXTRA = 1.8; // tweak 1.2..2.4 if needed
  const seg = clamped === max ? TOTAL + EXTRA : eased * (TOTAL + EXTRA);
...
export default function LoginModal({ open, onClose }) {
  ...
  const keypadKeysPhone = useMemo(() => ["1", "2", "3", "4", "5", "6", "7", "8", "9", "help", "0 / C", "blank"], []);
  const keypadKeysNumpad = useMemo(() => ["7", "8", "9", "4", "5", "6", "1", "2", "3", "help", "0 / C", "blank"], []);
  ...
  // Submit ring brighten progress (0..1)
          ...
          >
            ...
            {!(isMobile && useNativeKeyboard) && (
              <div
                ...
                style={{ position: "relative" }} // anchor for popover
              >
                    ...
                    return (
                      <button
                        ...
                        onPointerDown={(e) => {
                          ...
                          // no React state for 0/C swap — keep hooks stable
                  ...
                  <div
                    ...
                    style={{
                      ...
                      background: isLightTheme
                        ...
                        : "linear-gradient(145deg, rgba(255,255,255,0.08), rgba(0,0,0,0.38))",
                      boxShadow: isLightTheme
                        ? "0 16px 36px rgba(0,0,0,0.14), inset 0 0 0 1px rgba(17,24,39,0.10)"
                        : "0 16px 36px rgba(0,0,0,0.35), inset 0 0 0 1px rgba(255,255,255,0.10)",
                      color: isLightTheme ? "rgba(17,24,39,0.92)" : "rgba(255,255,255,0.92)",
                    }}
                  >
                    <button
                      type="button"
                    ...
                    <div style={{ paddingRight: "1.8rem" }}>
                      ...
                      <div style={{ marginTop: "0.45rem", fontSize: "0.95rem", lineHeight: 1.35, opacity: 0.92 }}>
                        Hoia <strong>0</strong> all ~0.5 s, siis saad kustutada, kui PIN läks valesti.
                      </div>
                    </div>
                  </div>
                )}
                ...
                <span className="login-submit-icon-stack" aria-hidden="true">
                  ...
                  {pinValue.length > 0 && (
                    <span className="login-submit-icon-overlay" aria-hidden="true">
                      <SubmitArcOverlayWhite size={80} filled={pinValue.length} max={PIN_MAX} idSuffix="login-submit" />
                    </span>
                  )}
                </span>
