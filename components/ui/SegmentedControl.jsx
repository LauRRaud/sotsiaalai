...
export default function SegmentedControl({
  ...
  return <div className={cn(groupStyles, className)} role="radiogroup" aria-disabled={disabled ? "true" : undefined}>
      ...
      return <label key={option.value} className={cn("flex items-center gap-[0.55rem] rounded-[var(--seg-card-radius)] [border-width:var(--seg-card-border-width,1px)] border-solid border-[color:var(--seg-card-border)] bg-[color:var(--seg-card-bg)] px-[0.9rem] py-[0.72rem] text-[1.03rem] font-normal tracking-[0.03em] text-[color:var(--seg-card-text)] shadow-[var(--seg-card-shadow)] transition-[color,border-color,background,box-shadow,transform] duration-150 ease-out", "hover:bg-[color:var(--seg-card-bg-hover)] hover:text-[color:var(--seg-card-text-hover)] hover:shadow-[var(--seg-card-shadow-hover)]", checked ? "bg-[color:var(--seg-card-bg-selected)] text-[color:var(--seg-card-text-selected)] shadow-[var(--seg-card-shadow-selected)]" : null, disabled ? "cursor-not-allowed opacity-70" : "cursor-pointer")}>
