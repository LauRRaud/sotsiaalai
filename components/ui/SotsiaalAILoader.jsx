...
export default function SotsiaalAILoader({
  ...
  const stageClass = [styles.stage, !animated && styles.static, className].filter(Boolean).join(" ");
  const px = size != null ? typeof size === "number" ? `${size}px` : String(size) : undefined;
  const red = {
    s0: redStops?.s0,
    s25: redStops?.s25,
    s50: redStops?.s50,
  ...
  return <div className={stageClass} {...accessibilityProps} data-animated={animated ? "true" : "false"} style={{
    "--size": px || "var(--sotsiaalai-loader-size, 44px)",
    width: px,
    height: px,
    "--glow": color,
    "--loader-red-0": red.s0,
    "--loader-red-25": red.s25,
    "--loader-red-50": red.s50,
    "--loader-red-75": red.s75,
    "--loader-red-100": red.s100,
    "--pulse": `${pulse}s`,
    ...
    ...style
      ...
      <svg viewBox="0 0 111.12 170.15" aria-hidden="true" className={styles.svg}>
        <defs>
          <style>{`.st0{fill:url(#linear-gradient2);stroke:none}.st1{fill:url(#red-vertical);stroke:none}.st2{fill:url(#linear-gradient);stroke:none}`}</style>
          ...
          <linearGradient id="linear-gradient" x1="24.12" y1="74.54" x2="24.12" y2="35.3" gradientTransform="translate(0 194.78) scale(1 -1)" gradientUnits="userSpaceOnUse">
            ...
            <stop offset=".47" stopColor="#111111" />
          ...
          <linearGradient id="red-vertical" x1="0" y1="0" x2="0" y2="1" gradientUnits="objectBoundingBox">
            <stop offset="0" stopColor="var(--loader-red-0,#cfa099)" />
            <stop offset="0.35" stopColor="var(--loader-red-25,#a4615c)" />
            <stop offset="0.7" stopColor="var(--loader-red-50,#6a302f)" />
            <stop offset="1" stopColor="var(--loader-red-100,#301413)" />
          </linearGradient>
        </defs>

        {showBottomBall && <g className={`${styles.ball} ${styles.bottom} send-loader-bottom`}>
            <path className="st2" d="M22.75,120.35c24.06-2.42,28.53,35.69,5,38.88-27.44,3.73-31.31-36.25-5-38.88h0Z" />
          </g>}
        ...
        <g className={`${styles.ball} ${styles.top}`}>
          <path className="st0" d="M75.38,8.47c27.75-4.37,32.82,36.21,3.92,36.76-24.7.47-25.74-33.32-3.92-36.76Z" />
        </g>
      </svg>
    </div>;
}
