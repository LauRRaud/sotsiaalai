...
export const optionEdgeGlowStyle = {
  "--edge-only-hot-end": "1.2%",
  "--edge-only-bright-end": "2.6%",
  "--edge-only-soft-end": "5.2%",
  "--edge-only-fade-end": "12%",
  "--edge-only-tail-end": "24%",
  "--edge-only-gap-start": "31%",
  "--edge-only-return-start": "69%",
  "--edge-only-return-soft": "78%",
  "--edge-only-return-bright": "88%",
  "--edge-only-return-hot": "96%",
...
export default function OptionCard({
  ...
  const textClassName = cn(
    fitTextLines
      ? "block min-w-0 flex-1 overflow-hidden leading-[inherit] [display:-webkit-box] [-webkit-box-orient:vertical] [backface-visibility:hidden] [-webkit-backface-visibility:hidden] [-webkit-font-smoothing:antialiased] [text-rendering:geometricPrecision] [transform:translateZ(0)]"
    ...
    fitTextLines === 1 ? "whitespace-nowrap text-[clamp(0.98rem,1.24vw,1.18rem)]" : null,
    fitTextLines && fitTextLines > 1 ? "text-[clamp(0.98rem,1.2vw,1.14rem)]" : null
  );
  ...
  const textStyle = fitTextLines
    ? {
        WebkitLineClamp: Math.max(1, Number(fitTextLines) || 1),
        fontSize: fitTextMaxPx
          ? `clamp(${Math.max(10, Number(fitTextMinPx) || 16)}px, 1.2vw, ${Math.max(Number(fitTextMinPx) || 16, Number(fitTextMaxPx) || Number(fitTextMinPx) || 16)}px)`
          : undefined
      }
    : undefined;
