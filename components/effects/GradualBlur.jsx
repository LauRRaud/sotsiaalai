...
const mergeConfigs = (...configs) => configs.reduce((acc, current) => ({ ...acc, ...current }), {});
  ...
  useEffect(() => {
    ...
    const calculate = () => {
      ...
      if (width <= 480 && config[`mobile${capitalizedKey}`]) {
        nextValue = config[`mobile${capitalizedKey}`];
      } else if (width <= 768 && config[`tablet${capitalizedKey}`]) {
        nextValue = config[`tablet${capitalizedKey}`];
      } else if (width <= 1024 && config[`desktop${capitalizedKey}`]) {
        nextValue = config[`desktop${capitalizedKey}`];
      }
...
function GradualBlur(props) {
  ...
  useEffect(() => {
    ...
    const timeoutMs = parseFloat(config.duration) * 1000;
  ...
  return (
    <div
      ref={containerRef}
      className={`gradual-blur ${config.target === "page" ? "gradual-blur-page gradual-blur-fixed" : "gradual-blur-parent"} ${config.className}`.trim()}
