...
export default function AnalyticsDashboard() {
  ...
  return <div className="rag-admin analytics-admin rag-admin--flat">
      <div className="rag-card-head analytics-head">
        ...
        <div className="rag-card-actions">
          <Button variant="primary" className="rag-btn" onClick={refresh} disabled={loadingSummary || loadingEvents}>
        ...
        <div className="card analytics-card">
          ...
          {groundingSummary ? <>
              <div className="analytics-bar">
                <span className="analytics-bar__segment analytics-bar__segment--strong" style={{
              width: `${groundingSummary.strong}%`
            }} />
                <span className="analytics-bar__segment analytics-bar__segment--ok" style={{
              width: `${groundingSummary.ok}%`
            }} />
                <span className="analytics-bar__segment analytics-bar__segment--weak" style={{
              width: `${groundingSummary.weak}%`
            }} />
              </div>
              <div className="analytics-meta">
                Tugev {groundingSummary.strong}% | OK {groundingSummary.ok}% |
                Nõrk {groundingSummary.weak}%
              </div>
            </> : <div className="analytics-meta">
              {loadingSummary ? "Laen..." : "-"}
            </div>}
          ...
          <table className="rag-table">
            ...
            <tbody>
              {loadingSummary ? <tr>
                ...
                </tr> : (summary?.ragDocs?.recent || []).length ? (summary?.ragDocs?.recent || []).map(d => {
          ...
          <table className="rag-table">
            ...
            <tbody>
              {loadingSummary ? <tr>
                ...
                </tr> : (summary?.billing?.recentPayments || []).length ? (summary?.billing?.recentPayments || []).map(p => <tr key={p.id}>
      ...
      <div className="card">
        ...
        <div className="rag-toolbar mt-2">
          <select className="input" value={eventFilter} onChange={e => setEventFilter(e.target.value)}>
            ...
            {EVENT_OPTIONS.map(ev => <option key={ev.value} value={ev.value}>
                {ev.label}
              </option>)}
          </select>
        ...
        <div className="table-wrap">
          <table className="rag-table">
            ...
            <tbody>
              {loadingEvents ? <tr>
                  <td colSpan={5}>Laen...</td>
                </tr> : filteredEvents.length ? filteredEvents.map(row => <tr key={row.id}>
