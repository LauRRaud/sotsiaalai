...
export default function AccessibilityModal({
  ...
  // This is now used as --csp-pad for real padding (fixes last-step reachability)
  ...
  // Compute padding so the active step can center (and last step becomes reachable)
  useEffect(() => {
  ...
  const {
    ...
    // Sujuvam scroll (väldi ratta "tõksumist")
    ...
    // Pehmem settlemine, et scroll ei hüppaks
    settleMs: 200,
    maxStepPerSettle: 1,
  ...
  // Center the step when focusing inside it
  useEffect(() => {
    const scrollEl = scrollRef.current;
    if (!scrollEl) return;
  ...
  return (
    <>
      ...
      >
        ...
        <div
          ref={scrollRef}
          className="a11y-modal__scroll csp-container"
          style={{ "--csp-pad": `${scrollPad}px` }}
          tabIndex={0}
          aria-label={t("profile.preferences.title")}
        >
          <fieldset className={`a11y-fieldset a11y-snap csp-step ${getItemClassName(0)}`}>
            ...
            <div className="a11y-options">
              <FancyRadio
                ...
                name="lg"
                value="et"
                label={t("accessibility.options.language.et")}
                checked={lang === "et"}
                onChange={() => setLang("et")}
                ref={firstFocusRef}
              />
              <FancyRadio
                ...
                name="lg"
                value="ru"
                label={t("accessibility.options.language.ru")}
                checked={lang === "ru"}
                onChange={() => setLang("ru")}
              />
              <FancyRadio
                ...
                name="lg"
                value="en"
                label={t("accessibility.options.language.en")}
                checked={lang === "en"}
                onChange={() => setLang("en")}
              />
            </div>
