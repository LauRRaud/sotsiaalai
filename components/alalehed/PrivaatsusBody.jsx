...
export default function PrivaatsusBody() {
  ...
  return <article className="main-content glass-box doc-page" aria-labelledby="privacy-title" lang={locale}>
      ...
      <section className="glass-section doc-body">
        {sections.map(section => <div key={section.heading}>
            ...
            {section.content.map((item, idx) => item.type === "list" ? <RichText key={`${section.heading}-list-${idx}`} as="ul" className="glass-list" value={item.value} replacements={item.replacements || lawLinkReplacements} /> : <RichText key={`${section.heading}-p-${idx}`} as="div" value={item.value} replacements={item.replacements || {}} />)}
          </div>)}
      </section>
