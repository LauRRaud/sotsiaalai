...
export default function UuendaEpostiBody() {
  ...
  return <InvitePageShell title={t("profile.email_update.title", "Uuenda e-post")} lang={locale} contentClassName="invite-page-content--wide" actionsClassName="invite-page-actions--side" actions={<button type="button" className="back-arrow-btn" onClick={handleBack} aria-label={backLabel}>
          <span className="back-arrow-circle" />
          <span className="sr-only">{backLabel}</span>
        </button>}>
      {submitted ? <div className="flex flex-col gap-4">
        ...
        </div> : <form className="flex flex-col gap-4" onSubmit={handleSubmit} autoComplete="off" aria-busy={loading ? "true" : "false"}>
          <label htmlFor="current-email" className="sr-only">
            {t("profile.email_update.current_placeholder", "Sinu e-post")}
          </label>
          <input type="email" id="current-email" name="current-email" className="invite-classic__input" placeholder={t("profile.email_update.current_placeholder", "Sinu e-post")} value={currentEmail} readOnly aria-readonly="true" autoComplete="email" inputMode="email" />
          <label htmlFor="email" className="sr-only">
            {t("profile.email_update.new_placeholder", "Uus e-post")}
          </label>
          <input type="email" id="email" name="email" className="invite-classic__input" placeholder={t("profile.email_update.new_placeholder", "Uus e-post")} value={email} onChange={e => setEmail(e.target.value)} required autoComplete="email" inputMode="email" disabled={loading} aria-invalid={error ? "true" : "false"} aria-describedby={errorId} />
          ...
          <input type="password" id="pin" name="pin" className="invite-classic__input" placeholder={t("profile.email_update.pin_placeholder", "Praegune PIN ({min}-{max} numbrit)", {
        min: PIN_MIN,
        max: PIN_MAX
      })} value={pin} onChange={e => setPin(e.target.value.replace(/\D/g, "").slice(0, PIN_MAX))} required minLength={PIN_MIN} maxLength={PIN_MAX} autoComplete="current-password" disabled={loading} />
