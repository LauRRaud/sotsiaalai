...
export default function UnustasinParooliBody() {
  ...
  return <InvitePageShell title={title} lang={locale} contentClassName="invite-page-content--lower invite-page-content--wide" actionsClassName="invite-page-actions--low invite-page-actions--side" actions={<button type="button" className="back-arrow-btn" onClick={handleBack} aria-label={backLabel}>
          <span className="back-arrow-circle" />
          <span className="sr-only">{backLabel}</span>
        </button>}>
      {submitted ? <RichText className="text-center text-[color:#a7f3d0]" as="div" value={t("auth.reset.success")} /> : <form className="flex flex-col gap-4" onSubmit={handleSubmit} autoComplete="off" aria-busy={loading ? "true" : "false"}>
          <label htmlFor="email" className="sr-only">
            {t("profile.email", "E-post")}
          </label>
          <input type="email" id="email" name="email" className="invite-classic__input" placeholder={t("auth.email_placeholder")} value={email} onChange={e => setEmail(e.target.value)} required autoComplete="username" disabled={loading} aria-invalid={error ? "true" : "false"} aria-describedby={errorId} />
