...
export default function ChatAiForwardToggle({
  ...
  return <div className={`${aiToggleRowClassName} ${className}`.trim()}>
    <label className={aiToggleLabelClassName}>
      <input type="checkbox" className={aiToggleInputClassName} style={aiToggleInputStyle} checked={sendToAssistant} onChange={e => setSendToAssistant(e.target.checked)} aria-describedby="chat-ai-hint" />
