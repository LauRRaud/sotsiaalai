...
export default function TypedFadeText({
  ...
  const content = (
    <span aria-hidden="true">
      {segments.map((ch, i) => (
        <span
          key={`${i}-${ch}`}
          className={["nav-meist-letter", ch === " " ? "is-space" : "", i < visibleCount ? "is-on" : ""]
            .filter(Boolean)
            .join(" ")}
        >
          {ch === " " ? "\u00A0" : ch}
  ...
  if (ariaHidden) {
    return (
      <span className={["nav-meist-text", isFading ? "is-fading" : "", className].filter(Boolean).join(" ")} aria-hidden="true">
        {content}
      </span>
    );
  }
  ...
  return (
    <span className={["nav-meist-text", isFading ? "is-fading" : "", className].filter(Boolean).join(" ")} aria-label={text}>
