...
export default function RoomsPage() {
  ...
  return <div className="relative flex min-h-[100dvh] w-full flex-col items-center justify-start pt-[calc(env(safe-area-inset-top,0px)+1rem)] pb-[env(safe-area-inset-bottom,0px)] max-[48em]:pt-[env(safe-area-inset-top,0px)] max-[48em]:pb-[env(safe-area-inset-bottom,0px)] min-[48em]:pt-[calc(env(safe-area-inset-top,0px)+clamp(0.7rem,1.9vh,1.3rem))]">
      <section className="main-content glass-box flex h-[var(--chat-diameter)] w-[var(--chat-diameter)] min-w-[var(--chat-diameter)] max-w-[var(--chat-diameter)] min-h-[var(--chat-diameter)] max-h-[var(--chat-diameter)] flex-col gap-4 overflow-auto overflow-x-hidden rounded-full p-[clamp(1.4rem,3.5vh,2.2rem)] aspect-square self-center mx-auto mt-[max(0px,calc((100dvh-var(--chat-diameter))/2-clamp(0.7rem,1.9vh,1.3rem)))] mb-0 max-[48em]:w-screen max-[48em]:h-[100dvh] max-[48em]:max-w-screen max-[48em]:max-h-[100dvh] max-[48em]:min-w-0 max-[48em]:min-h-0 max-[48em]:rounded-none max-[48em]:overflow-visible max-[48em]:aspect-auto max-[48em]:pt-[clamp(1.2rem,3vh,2rem)] max-[48em]:mt-0" role="region" aria-label={t("rooms.aria", "Ruumid")}>
        ...
        {loading ? <p className="mx-auto w-[min(28rem,90%)] text-center leading-[1.45] [.theme-light_&]:text-[color:var(--text-strong)]" aria-busy="true">
          ...
          </p> : <div className="flex-1 min-h-0 overflow-y-auto pb-[0.2rem] [scrollbar-width:none] [&::-webkit-scrollbar]:h-0 [&::-webkit-scrollbar]:w-0">
            <ul className="mx-auto grid w-[min(30rem,88%)] list-none gap-[0.85rem] p-0 text-left" role="list">
              {effectiveRooms.map(room => <li key={room.id} className="m-0 p-0">
                  <div className="flex flex-col gap-[0.6rem] rounded-[1rem] border border-[rgba(255,255,255,0.08)] bg-[rgba(10,14,24,0.32)] p-[0.75rem_0.85rem] text-[color:var(--pt-120)] shadow-[0_0.4rem_1rem_rgba(0,0,0,0.25)] transition-[transform,border-color,background,box-shadow] duration-150 hover:-translate-y-[1px] hover:border-[rgba(148,163,184,0.4)] hover:bg-[rgba(16,22,34,0.4)] hover:shadow-[0_0.55rem_1.35rem_rgba(0,0,0,0.32)] focus-within:-translate-y-[1px] focus-within:border-[rgba(148,163,184,0.4)] focus-within:bg-[rgba(16,22,34,0.4)] focus-within:shadow-[0_0.55rem_1.35rem_rgba(0,0,0,0.32)] [.theme-light_&]:border-[rgba(148,163,184,0.35)] [.theme-light_&]:bg-[rgba(255,255,255,0.85)] [.theme-light_&]:text-[#1f2937] [.theme-light_&]:shadow-[0_0.35rem_0.9rem_rgba(15,23,42,0.12)] [.theme-light_&:hover]:border-[rgba(148,163,184,0.55)] [.theme-light_&:hover]:bg-[rgba(255,255,255,0.96)] [.theme-light_&:hover]:shadow-[0_0.5rem_1.1rem_rgba(15,23,42,0.16)] [.theme-light_&:focus-within]:border-[rgba(148,163,184,0.55)] [.theme-light_&:focus-within]:bg-[rgba(255,255,255,0.96)] [.theme-light_&:focus-within]:shadow-[0_0.5rem_1.1rem_rgba(15,23,42,0.16)]">
                    <Link href={`/vestlus?roomId=${encodeURIComponent(room.id)}`} className="grid w-full gap-[0.45rem] text-inherit no-underline">
        ...
        <div className="back-btn-wrapper pointer-events-none absolute left-[calc(var(--hud-edge-left,0px)+clamp(0.1rem,1.2vw,0.8rem))] top-1/2 z-[100] m-0 -translate-y-1/2 p-0">
          <button type="button" className="back-arrow-btn pointer-events-auto !h-[5.7rem] !w-[5.7rem]" onClick={() => pushWithTransition(router, "/vestlus")} aria-label={t("rooms.back_to_chats", "Tagasi vestlustesse")}>
      ...
      {confirmRoom ? <div className="modal-backdrop" role="presentation" onClick={closeDeleteConfirm}>
          <div className="modal-confirm" role="dialog" aria-modal="true" aria-labelledby="rooms-delete-title" onClick={e => e.stopPropagation()}>
            ...
            <p className="modal-confirm-text">
              {t("rooms.delete_confirm", 'Kustuta ruum "{name}"? See kustutab sõnumid ja kutsed.').replace("{name}", confirmRoom.title || t("rooms.fallback_title", "Ruum"))}
            </p>
